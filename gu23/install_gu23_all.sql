...
-- Пакет ставится отдельно:   XX_DISL_GU23_PKG.pks + XX_DISL_GU23_PKG.pkb
  ...
  FOR x IN (
    SELECT 'DROP TABLE '||t||' CASCADE CONSTRAINTS PURGE' ddl FROM (
      SELECT column_value t FROM TABLE(sys.odcivarchar2list(
        ...
        'XX_DISL_GU23_REF_STATION','XX_DISL_GU23_REF_CARGO','XX_DISL_GU23_REF_REASON',
        'XX_DISL_GU23_REF_CEX','XX_DISL_GU23_USERS'))
    )
    UNION ALL
    SELECT 'DROP SEQUENCE '||s FROM (
      SELECT column_value s FROM TABLE(sys.odcivarchar2list(
        ...
        'XX_DISL_GU23_ACT_ROW_SEQ','XX_DISL_GU23_ACT_SEQ','XX_DISL_GU23_COUNTER_SEQ',
        'XX_DISL_GU23_REF_SIGNER_SEQ','XX_DISL_GU23_REF_WAGON_KIND_SEQ',
        'XX_DISL_GU23_REF_OWNER_SEQ','XX_DISL_GU23_REF_STATION_SEQ',
        'XX_DISL_GU23_REF_CARGO_SEQ','XX_DISL_GU23_REF_REASON_SEQ',
        'XX_DISL_GU23_REF_CEX_SEQ','XX_DISL_GU23_USERS_SEQ'))
    )
    UNION ALL
...
CREATE TABLE xx_disl_gu23_act (
    ...
    cargo_ref           VARCHAR2(256),             -- груз (справочное значение)
    reason              VARCHAR2(512),
...
SELECT a.id, a.act_number, a.act_type, a.status, a.cex_code,
       a.station, a.st_from, a.st_to, a.waybill_no, a.cargo_ref,
...
-- справочник: грузы
COMMENT ON TABLE  xx_disl_gu23_ref_cargo        IS 'Справочник грузов';
COMMENT ON COLUMN xx_disl_gu23_ref_cargo.id     IS 'ID груза (первичный ключ)';
COMMENT ON COLUMN xx_disl_gu23_ref_cargo.name   IS 'Наименование груза';
COMMENT ON COLUMN xx_disl_gu23_ref_cargo.active IS 'Признак активности: Y/N';
...
COMMENT ON COLUMN xx_disl_gu23_act.st_from         IS 'Станция отправления';
COMMENT ON COLUMN xx_disl_gu23_act.st_to           IS 'Станция назначения';
COMMENT ON COLUMN xx_disl_gu23_act.waybill_no      IS '№ накладной';
COMMENT ON COLUMN xx_disl_gu23_act.cargo_ref       IS 'Груз (из справочника)';
