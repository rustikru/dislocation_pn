...
as
  ...
  ******************************************************************************/
        ...
        select code
         ...
         where id = p_dept_id;
           ...
           update xx_disl_gu23_counter
              set cnt = cnt + 1
            where dept_id = p_dept_id and yr = v_yr
        returning cnt
             into v_cnt;
    ...
    is
        l_row   xx_disl_gu23_ref_row;
    begin
        for r in (  select st_code as st_code, st_name as name
    ...
    begin
        for r
            in (select fr_code_etsng as code, fr_name as name
                  from etw_nsi_freight
                 where     TRUNC (SYSDATE) between recdatebegin
                                               and recdateend
                                   ...
                                   where     (   p_type is null
                                         ...
                                         and (   (    v_from is null
                                              ...
                                              or (    a.start_at is not null
                                                  and (   v_from is null
                                                       or nvl(a.start_at,v_from) >= v_from)
                                                  and (   v_to is null
                                                       or nvl(a.start_at,v_to) < v_to))
                                              or (    a.end_at is not null
                                                  and (   v_from is null
                                                       or nvl(a.end_at,v_from) >= v_from)
                                                  and (   v_to is null
                                                       or nvl(a.end_at,v_to) < v_to))
                                              or (a.start_at is null and a.end_at is null))
    ...
    begin
        for a in (  select *
                      from xx_disl_gu23_act_v a
                     where a.act_type = 'start' and a.status in (  --'active',
                                                                 'signed')
                  order by a.start_at)
        loop
            pipe row (g_act_row (a));
    ...
    begin
        ...
        for r
            in (  select st_code, upper(st_name) st_name, st_id
        ...
        for r
            in (                       -- подписанты предприятия (stype='own')
                select u.id                                 as approver_id,
                       u.full_name,
                       LOWER (u.login) || '@company.ru'     as email
                  from xx_disl_gu23_signer  s
