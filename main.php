					...
					<div>
						<div class="add-btn-window">
							<div class="add-btn-titlebar helper-clearfix">
								...
								<button id="add_btn_turn" title="Скрыть список кнопок"
									class="add-btn-button add-btn-titlebar-turn-button">
							...
							<div id="add_btn_content" class="add-btn-content" style="display: none;">
								<?php if (($auth->getStationId() === '1' || $auth->getStationId() === '2') && $rights['shuntingOperation'] === '1') { ?>
									<input class="btnFind" style="display: none;" type="button" size="30"
										value="Обновить данные из КИС" onclick="updateCarsForUgleuralskaya()">
								<?php } ?>
								<?php if ($rights['enter_inspection_add'] === 'Y' || $rights['enter_inspection'] === 'Y') { ?>
									<input id="inspections_btn" class="btnFind" type="button" size="15" value="Осмотры"
										onclick="create_md_inspections();">
								<?php } ?>
								<?php if ($rights['enter_dev_inspection'] === 'Y') { ?>
									<input id="inspections_btn_gu" class="btnFind" type="button" size="15" value="Осмотры ЗУ"
										onclick="create_md_fix_device();">
								<?php } ?>
								<?php if ($rights['weigh_import'] === 'Y') { ?>
									<input id="weigh_import_btn" class="btnFind" type="button" size="20"
										value="Импорт взвешиваний" onclick="create_md_weigh_import();">
								<?php } ?>
								<?php if ('Y' === 'Y') { ?>
									<input id="export_shop_info_btn" class="btnFind" type="button" size="20" value="OEBS налив"
										onclick="create_md_export_shop_info();">
								<?php } ?>
								<?php if ('Y' === 'Y') { ?>
									<input id="export_gmd_samples_btn" class="btnFind" type="button" size="20"
										value="OEBS проба" onclick="create_md_export_samples();">
								<?php } ?>
								<?php if ('1' === '1') { ?>
									<input id="weigh_result_btn" class="btnFind" type="button" size="15" value="Взвешивания"
										onclick="create_md_weight_result();">
								<?php } ?>
								<?php if ('1' === '1') { ?>
									<input id="car_passport_btn" class="btnFind" type="button" size="15" value="Паспорт вагона"
										onclick="create_md_cars_passport(false);">
								<?php } ?>
								<?php if ($rights['process_of_wagons'] === 'Y') { ?>
									<input id="processing_wagons_btn" class="btnFind" type="button" size="15"
										value="Обработка вагонов" onclick="get_processings_of_wagons();">
								<?php } ?>

								<?php if ($auth->getStationId() === '2' && $rights['entry_foreign_car'] === 'Y') { ?>
									<input id="entry_foreign_railcar_btn" class="btnFind" style="display:none" type="button"
										size="15" value="Ввод чужих вагонов" onclick="entry_foreign_railcar();">
								<?php } ?>
								<?php if ($rights['entry_foreign_cont'] === 'Y') { ?>
									<input id="entry_foreign_container_btn" class="btnFind" type="button" size="15"
										value="Ручной ввод контейнеров" onclick="entry_foreign_container();">
								<?php } ?>
								<?php if ($auth->getStationId() === '2' && $rights['register_notification'] === 'Y') { ?>
									<input id="notification_btn" class="btnFind" style="display:none" type="button" size="15"
										value="Регистрация уведомлений" onclick="create_modal_dialog_notification();">
								<?php } ?>

								<?php if (($auth->getStationId() === '2' && $rights['register_notification_gu'] === 'Y') || ($rights['administrator'] === 'Y')) { ?>
									<input id="notification_gu_btn" class="btnFind" type="button" size="15"
										value="Регистрация Уведомление ГУ-2б"
										onclick="create_modal_dialog_notification_gu(null, '2b');">
									<input id="notification_gu_2d_btn" class="btnFind" type="button" size="15"
										value="Регистрация Уведомление ГУ-2д"
										onclick="create_modal_dialog_notification_gu(null, '2d');">
								<?php } ?>
								<?php if (($auth->getStationId() === '2' && $rights['export_notification_gu'] === 'Y') || ($rights['administrator'] === 'Y')) { ?>
									<input id="open_notification_gu_btn" class="btnFind" type="button" size="15"
										value="Уведомления ГУ-2б" onclick="create_modal_dialog_list_notification_gu('2b');">
									<input id="open_notification_gu_2d_btn" class="btnFind" type="button" size="15"
										value="Уведомления ГУ-2д" onclick="create_modal_dialog_list_notification_gu('2d');">
								<?php } ?>
							</div>
						</div>
						<div class="contex-container">
							...
							<ul class="tree_Container" id="currentCarstree">
								<li data-id="<?php echo $auth->getStationId(); ?>" data-type="station"
									class="tree_Node tree_IsRoot tree_IsLast tree_ExpandOpen">
									...
									<div class="tree_Content">
										...
										<select id="selectStation" class="selectStationClass">
											<?php foreach ($helper->returnStations() as $value) { ?>
												<option value="<?php echo $value['ID'] ?>" <?php echo $value['ID'] == $auth->getStationId() ? 'selected' : ''; ?>>
							...
							<?php if ($auth->getStationId() !== '1' && $rights['receive_to_station'] === 'Y') { ?>
								<input id="received_into_station_btn" class="btnFind" type="button" size="15"
									value="Приём по списку" onclick="create_md_received_into_station_new();">
							...
							<input id="fillRailcarForInvoice" class="btnFind" type="button" size="15"
								value="Добавить номер вагона" onclick="fill_railcar_for_invoice();">
							<?php /*}*/ ?>
							<ul class="tree_Container" id="comingCarsTree">
								<li data-id="<?php echo $auth->getStationId(); ?>" data-type="station"
									...
									<div class="tree_Expand"></div>
									<div class="tree_Content">
										<div id="refreshComingRailcar" class="refresh" title="Обновить приходящие вагоны">
										</div>
										Подход вагонов
									</div>
