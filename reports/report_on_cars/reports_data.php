...
if ($_POST['ajax_action']==='get_report_4_data') {
    $conn = oci_connect($user,$pwd,$db,"AL32UTF8");
    if (!$conn) {
            $e = oci_error();
            trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
    ...
    $arrChild = array();
    $oci_child = oci_parse($conn, 'select * from table(xx_etw.xx_dislocation.get_report_4_data(:bind1)) order by car_number');
	OCIBindByName($oci_child, ":bind1", filter_input(INPUT_POST,'p_add_data'));
    oci_execute($oci_child);
    while ($tmp = oci_fetch_array($oci_child, OCI_ASSOC+OCI_RETURN_NULLS)) {
            array_push($arrChild, $tmp);
    }
