...
var g_selected_railcar =
  [] /*Массив выбранных вагонов (для сохранения порядка выбора)*/
...
$(document).ready(function () {
  ...
  $(document).on('click', function (event) {
    if (
      !$(event.target).closest($accountButton).length &&
      !$(event.target).closest($accountMenu).length
    ) {
      $accountMenu.removeClass('active') // Скрываем меню
    }
  })
  ...
  $.ajax({
    ...
    error: function (xhr) {
      window.alert('Ошибка загрузки данных: ' + xhr.status)
    },
  })
  ...
  $('#currentCarstree,#comingCarsTree').mousedown(function (event) {
    ...
    if (event.which === 3) {
      /*&&(cur_cars_tree_station === user_station_id ||(user_station_id==='2'&&cur_cars_tree_station==='1'))*/
      ...
      if (
        target.parents('#currentCarstree').length !== 0 &&
        user_station_id !== '1'
      ) {
        //ищем родителей с классом #cur_station
        if (target.hasClass('tree_Content')) {
          ...
          if (!target.hasClass('tree_selected')) {
            addInfoAjax(
              clickedLi,
              false,
              $('#selectStation > option:selected').val(),
            )
            $('.tree_selected').removeClass('tree_selected')
            target.addClass('tree_selected')
          }

          var l_flag_same_parent_selected_railcar =
            get_flag_same_parent_selected_railcar()

          var l_flag_selected_railcars_not_have_notification =
            get_flag_selected_railcars_not_have_notification()

          if (
            cur_cars_tree_station === user_station_id ||
            user_admin /*add 10.09.2024 BekmansrurovRR*/
          ) {
            if (
              r_moving_inside_railway &&
              l_flag_same_parent_selected_railcar
            ) {
              if (
                (clickedDataType === 'bandwagon' ||
                  clickedDataType === 'railcar' ||
                  clickedDataType === 'cont') &&
                $('.tree_selected').length === 1
              ) {
            ...
            if (
              r_moving_inside_station &&
              ((clickedDataType === 'railcar' &&
                l_selected_obj.length === 1 &&
                l_selected_obj[0].cont != '') ||
                (clickedDataType === 'cont' &&
                  get_type_selected_elements() == 'cont'))
            ) {
              ul.append(
                '<li data-action="move_cont">Переместить контейнера</li>',
              )
            }
            ...
            if (l_flag_same_parent_selected_railcar) {
              if (
                ...
                ul.append(
                  '<li data-action="move">Переместить внутри станции</li>',
                )
              }
              if (
                r_moving_between_station &&
                (clickedDataType === 'bandwagon' ||
                  clickedDataType === 'railcar')
              ) {
                ul.append(
                  '<li data-action="sendToStation">Отправить на другую станцию</li>',
                )
              }
              if (
                r_work_with_groups &&
                (clickedDataType === 'railway' ||
                  clickedDataType === 'bandwagon')
              ) {
              ...
              if (
                ...
                ul.append(
                  '<li data-action="deleteBandwagon">Удалить группу</li>',
                )
              }
              if (
                r_register_notification_gu &&
                cur_cars_tree_station === '2'
                //|| user_admin /*add 10.09.2024 BekmansrurovRR*/
              ) {
                ul.append(
                  '<li data-action="register_notification_gu">Регистрация уведомлений ГУ</li>',
                )
              }
              if (
                r_output_defective_cars &&
                cur_cars_tree_station === '3' &&
                l_selected_obj.length !== 0 &&
                ...
                ul.append(
                  '<li data-action="output_cars">Вывод. Вагон разобран</li>',
                )
              }
              if (clickedDataType === 'railway') {
                ...
                //if (r_fix_dev_undock){
                ul.append(
                  '<li data-action="undock_fix_device">Снять закрепление ЗУ</li>',
                )
                //}
                //if (r_fix_dev_update){
                ul.append(
                  '<li data-action="update_fix_device">Корректировка закреплений ЗУ</li>',
                )
                //}
              }
            }
            ...
            user_station_id === '2' &&
            cur_cars_tree_station === '1' &&
            l_flag_selected_railcars_not_have_notification
          ) {
            ul.append(
              '<li data-action="register_notification">Регистрация уведомлений</li>',
            )
          }

          if (clickedDataType === 'railway' || clickedDataType === 'area') {
            ul.append(
              '<li data-action="toggle_like_railway">Доб/убрать любимый путь</li>',
            )
          }
        }
      } else if (target.parents('#comingCars').length !== 0) {
        //ищем родителей с классом #comingCars
        if (target.hasClass('tree_Content')) {
          ...
          var l_flag_selected_railcars_not_have_notification =
            get_flag_selected_railcars_not_have_notification()
          var l_flag_can_received_selected_railcars =
            get_flag_can_received_selected_railcars()
          console.log(
            'l_flag_same_parent_selected_railcar=' +
              l_flag_same_parent_selected_railcar,
          )
          ...
          console.log(
            'l_flag_can_received_selected_railcars=' +
              l_flag_can_received_selected_railcars,
          )
          ...
          if (
            (l_flag_same_parent_selected_railcar &&
              cur_cars_tree_station === '1' &&
              r_out_from_ugl &&
              $('.tree_selected').parent('li[data-from_station_id="-1"]')
                .length === 0) ||
            user_admin /*add 03.03.2026 BekmansurovRR*/
          ) {
      ...
      if (r_change_attribute) {
        ...
        if (
          ...
          ul.append(
            '<li data-action="change_cont_attr_in_car">Изменить аттрибуты контейнера</li>',
          )
        }
        /*Изменяем атрибуты контейнера*/
        if (clickedDataType === 'cont' && $('.tree_selected').length === 1) {
          ul.append(
            '<li data-action="change_cont_attr">Изменить аттрибуты контейнера</li>',
          )
        }
      }
      ...
      if (
        r_change_weight &&
        !exists_platform_without_conts &&
        l_flag_same_parent_selected_railcar &&
        (clickedDataType === 'railcar' || clickedDataType === 'cont')
      ) {
        ul.append(
          '<li data-action="change_cars_weight_net">' +
            (l_state == 'empty' ? 'Погрузка' : 'Разгрузка') +
            '</li>',
        )
      }
      ...
      if (
        ...
        if (l_state != 'empty') {
          ul.append(
            '<li data-action="change_cars_weight_net_akm">' +
              (l_state == 'empty'
                ? 'Погрузка при перемещении(АКМ)'
                : 'Разгрузка при перемещении(АКМ)') +
              '</li>',
          )
        }
      }
      ...
      if (
        ...
        l_selected_obj.length === 1 &&
        clickedDataType === 'railcar' &&
        l_selected_obj[0].railcar_type === '1'
      ) {
        ul.append(
          '<li data-action="change_pl_cars_weight_net">Постановка платформы</li>',
        )
      }
      ...
      if (
        ...
        $('.tree_selected').length === 1 &&
        (target.parents('#currentCarstree').length !== 0 ||
          target.parents('#comingCars').length !== 0)
      ) {
      ...
      if (
        ...
        r_enter_inspection &&
        clickedDataType === 'railcar' &&
        get_type_selected_elements() == 'railcar' &&
        (target.parents('#currentCarstree').length !== 0 ||
          target.parents('#comingCars').length !== 0)
      ) {
        ul.append(
          '<li data-action="processing_of_wagons">Обработка вагонов</li>',
        )
      }
      if (
        r_enter_inspection &&
        ...
        ul.append(
          '<li data-action="enter_inspection_for_few_cars">Ввод осмотра нескольких вагонов</li>',
        )
      }
      ...
      if (
        ...
        ul.append(
          '<li data-action="enter_inspection_for_few_conts">Ввод осмотра нескольких контейнеров</li>',
        )
      }
      ...
      if (
        r_create_invoice_out &&
        $('.tree_selected').length === 1 &&
        get_type_selected_elements() == 'railcar' &&
        get_flag_can_create_return_invoice() === 'Y'
      ) {
        ul.append(
          '<li data-action="create_return_invoice">Создать накладную на возврат</li>',
        )
      }
      ...
      if (target.parents('#cur_station, #comingCars').length !== 0) {
        if (target.hasClass('tree_Content')) {
          ...
          if (
            parentTarget.attr('data-type') === 'bandwagon' ||
            parentTarget.attr('data-type') === 'railway' ||
            parentTarget.attr('data-type') === 'point' ||
            parentTarget.attr('data-type') === 'area'
          ) {
            if (target.hasClass('tree_selected'))
              parentTarget
                .children('ul')
                .find('.tree_Content')
                .removeClass('tree_selected')
            else
              parentTarget
                .children('ul')
                .find('.tree_Content')
          ...
          if (
            parentTarget.attr('data-type') === 'railcar' ||
            parentTarget.attr('data-type') === 'cont' ||
            parentTarget.attr('data-type') === 'bandwagon' ||
            parentTarget.attr('data-type') === 'point' ||
            parentTarget.attr('data-type') === 'area'
          ) {
            if (target.hasClass('tree_selected')) {
              parentTarget
                .parents('li[data-type="bandwagon"]')
                .children('.tree_Content')
                .removeClass('tree_selected')
              parentTarget
                .parents('li[data-type="railway"]')
                .children('.tree_Content')
                .removeClass('tree_selected')
              parentTarget
                .parents('li[data-type="area"]')
                .children('.tree_Content')
                .removeClass('tree_selected')
            }
          }
          ...
          var l_flag_selected_cars_on_station =
            get_flag_selected_cars_on_station()
          ...
          if (
            ...
            clickedDataType === 'railcar' ||
            clickedDataType === 'railway' ||
            clickedDataType === 'point' ||
            clickedDataType === 'area' ||
            clickedDataType === 'cont'
          ) {
            if (target.hasClass('tree_selected')) {
              addInfoAjax(
...
function get_selected_objects() {
  ...
  $('table.addInfoTable tbody tr:not(:last)').each(function () {
    ...
    l_car.can_create_return_invoice = $(this)
      .children('td:nth-child(21)')
      .text()
...
function get_flag_same_parent_selected_railcar() {
  ...
  $('.tree_selected').each(function (index) {
    ...
    cur_location =
      $(this).parents('ul#currentCarstree').length === 1
        ? 'in_station'
        : 'coming_to_station'

    if (
      (prev_station_id !== cur_station_id || prev_location !== cur_location) &&
...
function get_flag_selected_railcars_not_have_notification() {
  ...
  $('.tree_selected').each(function (index) {
    if (
      $(this).parent().attr('data-notification') === 'Y' ||
      $(this).parent().attr('data-from_station_id') === '2'
    ) {
      l_flag = false
      return
    }
...
function get_car_containers(p_car_number) {
  ...
  $.ajax({
    ...
    dataType: 'text',
    async: false,
    data: { car_number: p_car_number, ajax_action: 'get_car_containers' },
    success: function (data) {
...
function get_flag_can_received_selected_railcars() {
  ...
  $('.tree_selected').each(function (index) {
    if (
      ($(this).parent().attr('data-from_station_id') === '1' &&
        $(this).parent().attr('data-notification') === 'N') ||
      $(this).parents('#comingCars').length === 0
    ) {
      l_flag = false
      return
    }
...
function contextMenuAction1lvl(event, p_clicked_li) {
  ...
  switch (action) {
    ...
    case 'sendToStation':
      ...
      create_contect_menu_2lvl_send_to_station(
        target.offset().left + target.innerWidth(),
        target.offset().top,
      )
      break
    ...
    case 'change_cont_attr_in_car':
      $('.context-menu-2lvl').remove()
      /*Создаем контексное меню 2-ого уровня для отправки на соседние станции*/
      create_contect_menu_2lvl_change_cont_attr(
        target.offset().left + target.innerWidth(),
        target.offset().top,
        p_clicked_li,
      )
    ...
    case 'add_fix_device':
      $('.context-menu-2lvl').remove()
      create_contect_menu_2lvl_add_fix_device(
        p_clicked_li.attr('data-id'),
        target.offset().left + target.innerWidth(),
        target.offset().top,
      )
      break
    ...
    case 'undock_fix_device':
      $('.context-menu-2lvl').remove()
      create_contect_menu_2lvl_undock_fix_device(
        p_clicked_li.attr('data-id'),
        target.offset().left + target.innerWidth(),
        target.offset().top,
      )
    ...
    case 'update_fix_device':
      $('.context-menu-2lvl').remove()
      create_contect_menu_2lvl_update_fix_device(
        p_clicked_li.attr('data-id'),
        target.offset().left + target.innerWidth(),
        target.offset().top,
      )
...
function create_contect_menu_2lvl_change_cont_attr(p_x, p_y, p_clicked_li) {
  if ($('#context_menu_2lvl_change_cont_attr').length === 0) {
    $.ajax({
      ...
      async: false,
      data: {
        car_number: p_clicked_li.attr('data-id'),
...
function md_change_cars_weight_net_new(p_type, p_loading_subs) {
  ...
  var date_post_input = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all required',
  }).attr('size', '15')
  var date_start_input = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
  }).attr('size', '15')
  var date_end_input = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
  }).attr('size', '15')
  var date_zayavka_uvod_input = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
  }).attr('size', '15')
  var date_uvod_input = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
  }).attr('size', '15')
  ...
  function md_change_cars_weight_net_ajax(
    p_cars_with_weight,
    p_date_post,
    p_date_start,
    p_date_end,
    ...
    $.ajax({
      ...
      async: false,
      data: {
        cars_with_weight: p_cars_with_weight,
  ...
  function railcar_table_const_local() {
    ...
    return_table.head = $('<table>', { class: 'change_weight_table' }).append(
      $('<tbody>'),
    )
    ...
    return_table.head.append(return_table.tr)

    return_table.append(return_table.head)
    return_table.cars_table = $('<table>', {
      class: 'change_weight_table',
    }).append($('<tbody>'))
    return_table.append(
      $('<div>', {
    ...
    return_table.cars_table_total_row = $('<tr>', {
      css: { background: '#EBEBEB', 'font-weight': 'bold' },
    })
    ...
    return_table.append(
      $('<table>', {
        class: 'change_weight_table',
        css: { 'margin-top': '-4px' },
      }).append($('<tbody>').append(return_table.cars_table_total_row)),
    )
    ...
    this.add_cars_in_table = function (
      ...
      tr.scales_type = $('<select>', {
        class: 'scales_type',
        css: { width: '100px' },
      })
      tr.define_task = $('<select>', {
        class: 'weight_net',
        css: { width: '85px' },
      })
      ...
      tr.weight_net_input = $('<input>', {
        class: 'weight_net',
        type: 'text',
        maxlength: '6',
      }).val(p_weight_net.replace(',', '.'))
      tr.accepted_weight = $('<input>', {
        class: 'accepted_weight',
        type: 'text',
        maxlength: '6',
      }).val(p_accepted_weight)
      tr.weight_dep = $('<input>', {
        class: 'weight_dep',
        type: 'text',
        maxlength: '6',
      }).val(p_weight_dep.replace(',', '.'))
      //tr.weight_dep = p_weight_dep.replace(',','.');
      //tr.weight_gross_td = $('<td>').text(p_weight_gross.replace(',','.'));
      tr.weight_gross_td = $('<input>', {
        class: 'weight_gross_td',
        type: 'text',
        maxlength: '6',
      }).val(p_weight_gross.replace(',', '.'))
      tr.refusal_input = $('<input>', {
        type: 'text',
        maxlength: '200',
        css: { width: '100px' },
      }).val(p_refusal)
      ...
      tr.scales_type = get_select_scales_type_list(
        tr.scales_type,
        r_scale_type_1831_manual,
        p_otgruzka_1c_id,
      )
      tr.define_task = get_select_define_task_list(
        tr.define_task,
        p_define_task,
      )
    ...
    this.add_selected_cars_rows_for_table = function (
      p_objects,
      p_only_cars,
      p_loading_subs,
    ) {
        ...
        if (item.checkbox.prop('checked')) {
          ...
          if (weight_dep < 0) {
            create_info_modal_dialog_new(
              'Оповещение',
              'У вагона ' + car_num + ' тара меньше 0!',
            )
          ...
          if (car_weight > 100) {
            create_info_modal_dialog_new(
              'Оповещение',
              'У вагона ' + car_num + ' вес больше 100!',
            )
            validation_save = false
            return false
          }
          ...
          if (weight_dep > 40) {
            create_info_modal_dialog_new(
              'Оповещение',
              'У вагона ' + car_num + ' тара больше 40!',
            )
            validation_save = false
            return false
          }
          ...
          if (define_task == 1 && p_loading_subs == 'un-loading-akm') {
            create_info_modal_dialog_new(
              ...
              'У вагона ' +
                car_num +
                ' не выбрано значение для столбца "Определить задачу"!',
            )
            validation_save = false
            return false
          }
          ...
          if (
            car_weight == 0 &&
            (accepted_weight == '' || accepted_weight == null) &&
            p_loading_subs == 'un-loading-akm'
          ) {
            create_info_modal_dialog_new(
              'Оповещение',
              'У вагона ' +
                car_num +
                ' не выбрано значение для столбца "Вес принят, т"!',
            )
            validation_save = false
            return false
          }
    ...
    this.mark_same_platform = function () {
      ...
      tr_mas.forEach(function (item, index, arr) {
        ...
        next_car_number =
          arr.length - 1 === index ? '' : arr[index + 1].car_number

        if (
          prev_car_number == item.car_number &&
          item.car_number == next_car_number &&
          next_car_number != '' &&
          prev_car_number != ''
        ) {
  ...
  function fill_select_with_history(p_select, p_cars_from_shop_mas) {
    ...
    p_cars_from_shop_mas.forEach(function (item, index) {
      options +=
        '<option style="' +
        (item.selected_car_in === 'Y' ? 'background:yellow' : '') +
        '" value="' +
        index +
        '">' +
        item.date_post +
  ...
  function change_label_text(p_state, p_loading_subs) {
    ...
    who_start_label.text(
      (p_state == 'empty' ? 'Погрузку' : 'Разгрузку') + ' начал',
    )
    who_end_label.text(
      (p_state == 'empty' ? 'Погрузку' : 'Разгрузку') + ' закончил',
    )
    $('#modalDialog,#ui-id-1').attr(
      'title',
      (p_state == 'empty' ? 'Погрузка' : 'Разгрузка') +
        ' вагонов' +
        (p_loading_subs == 'un-loading-akm' ? ' при перемещении.' : '-'),
    )
  ...
  function disable_save_btn() {
    if (
      date_post_input.hasClass('red_bckg_color') ||
      date_post_input.val() == '' ||
  ...
  function get_select_freight_list(p_freight) {
    ...
    result_select.append(
      '<option selected value="' + p_freight + '">' + p_freight + '</option>',
    )
  ...
  function get_cont_out_date(p_conts) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { conts: p_conts, ajax_action: 'get_cont_out_date' },
      success: function (data) {
        res = data
      },
      error: function (data) {
  ...
  function get_railcar_in_date(p_cars) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { cars: p_cars, ajax_action: 'get_railcar_in_date' },
      success: function (data) {
        res = data
      },
      error: function (data) {
  ...
  function get_move_cont_to_pl(p_cars) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { cars: p_cars, ajax_action: 'get_move_cont_to_pl' },
      success: function (data) {
        res = data
      },
      error: function (data) {
  ...
  md_div.append(
    $('<div>', { css: { display: 'table' } }).append(
      $('<div>', { class: 'attr', css: { border: 'none', width: '490px' } })
        ...
        .append($('<div>').append(date_start_label).append(date_start_input))
        .append($('<div>').append(date_end_label).append(date_end_input))
        .append(
          $('<div>')
        ...
        .append($('<div>').append(who_start_label).append(who_start_select))
        .append($('<div>').append(who_end_label).append(who_end_select))
        .append(
          $('<div>')
  ...
  var l_selected_car_cont_mas = railcar_table.add_selected_cars_rows_for_table(
    l_selected_objects,
    p_type === 'platform' ? '1' : '0',
    p_loading_subs,
  )
  ...
  $.ajax({
    ...
    async: false,
    data: {
      return_type: p_type === 'platform' ? '1' : '0',
  ...
  l_cars_from_shop.forEach(function (item) {
    var l_car = {
      ...
      weight_dep: item.WEIGHT_DEP,
      weight_gross: item.WEIGHT_GROSS,
      arrive_weight_net: item.ARRIVE_WEIGHT_NET,
    ...
    var l_selected_car_flag =
      item.CONT_NUMBER == null
        ? l_selected_car_cont.indexOf(item.CAR_NUMBER)
        : l_selected_car_cont.indexOf(item.CONT_NUMBER)
    if (l_prev_id != item.ID) {
      l_cars_from_shop_mas[item.ID] = {
        date_post: item.DATE_POST,
        date_start: item.DATE_START,
        date_end: item.DATE_END,
        date_zayavka_uvod: item.DATE_ZAYAVKA_UVOD,
        date_uvod: item.DATE_UVOD,
        created_by: item.CREATED_BY,
        who_looked: item.WHO_LOOKED,
        who_start: item.WHO_START,
        who_end: item.WHO_END,
        who_zayavka: item.WHO_ZAYAVKA,
        selected_car_in: 'N',
        cars: [l_car],
      }
    } else {
  ...
  l_tmp_cars.forEach(function (item) {
    if (item.cont_number != '') {
      l_conts_in_table += item.cont_number + '|'
    }
    if (p_type === 'platform' && item.car_number != '') {
      l_platforms_in_table += item.car_number + '|'
    }
    var l_car = {
      car_number: item.car_number,
      cont_number: item.cont_number,
      freight_name: item.freight_select.val(),
      weight_net: item.weight_net_input.val(),
      weight_dep: item.weight_dep,
      weight_gross: item.weight_gross_td.text(),
  ...
  history_select.on('change', { p_cars: l_cars_from_shop_mas }, function (e) {
    ...
    l_history.cars.forEach(function (item, i) {
      railcar_table.add_cars_in_table(
        ...
        item.arrive_weight_net !== null
          ? item.arrive_weight_net.replace(',', '.')
          : '',
        item.refusal !== null ? item.refusal : '',
        item.shop_info_id,
        p_type === 'platform' ? '1' : '0',
  ...
  date_post_input.blur(function () {
    disable_save_btn()
  })
  date_start_input.blur(function () {
    disable_save_btn()
  })
  date_end_input.blur(function () {
    disable_save_btn()
  })
  date_zayavka_uvod_input.blur(function () {
    disable_save_btn()
  })
  date_uvod_input.blur(function () {
    disable_save_btn()
  })
      ...
      Сохранить: {
        ...
        click: function () {
          if (date_end_input.val() != '') {
            if (
              date_comparison(date_post_input.val(), date_end_input.val(), '>')
            ) {
              create_info_modal_dialog_new(
                'Оповещение',
                '"Нач. погрузки (местное)" больше/равно "Кон. погрузки (местное)"',
              )
              return
            }
          }
          ...
          if (
            date_uvod_input.val() != '' &&
            check_open_period('2', date_uvod_input.val()) == '0'
          ) {
            create_info_modal_dialog_new(
              'Оповещение',
              'Для даты ' +
                date_uvod_input.val() +
                ' нет открытого периода! Операции сохранить не возможно!',
            )
          } else {
            ...
            //return;
            var l_cars_for_request =
              railcar_table.get_cars_for_request_in_table()

            //console.log('l_cars='+l_cars);

            var f_res = md_change_cars_weight_net_ajax(
              l_cars,
              date_post_input.val(),
              date_start_input.val(),
              date_end_input.val(),
              date_zayavka_uvod_input.val(),
              date_uvod_input.val(),
              who_looked_select.val(),
              who_start_select.val(),
              who_end_select.val(),
            ...
            if (f_res_mas[0] == 'done') {
              ...
              create_info_modal_dialog_new(
                'Ошибка',
                'Процедура завершилась с ошибкой!' + f_res_mas[1],
              )
              return
            }
...
function toggle_like_railway(p_clicked_li) {
  ...
  if (
    toggle_like_railway_ajax(
      p_clicked_li.attr('data-id'),
      p_clicked_li.attr('data-type'),
    ) === 'done'
  ) {
...
function railcar_table_const() {
  ...
  return_table.add_carnumber_input = $('<input>', {
    type: 'text',
    css: { 'font-size': '11px' },
    class: 'text ui-widget-content ui-corner-all',
  })
  ...
  return_table.add_carnumber_btn = $('<input>', {
    type: 'button',
    css: { 'font-size': '11px', height: '17px' },
    class: 'btnAdd',
  }).val('Добавить')
  ...
  function change_cars_table_total_tr() {
    ...
    if (self.cars_count !== 0) {
      ...
      return_table.cars_table_total_row.append('<td></td>')
      return_table.cars_table_total_row.append('<td></td>')
      return_table.cars_table_total_row.append(
        '<td>Кол-во: ' + self.cars_count + '</td>',
      )
      return_table.cars_table_total_row.append('<td></td>')
      return_table.cars_table_total_row.append(
        '<td>' + Math.round(sum_weight_net * 100) / 100 + '</td>',
      )
      return_table.cars_table_total_row.append(
        '<td>' + Math.round(sum_weight_dep * 100) / 100 + '</td>',
      )
      return_table.cars_table_total_row.append(
        '<td>' + Math.round(sum_weight_gross * 100) / 100 + '</td>',
      )
      return_table.cars_table_total_row.append(
        '<td>' + Math.round(sum_car_length * 100) / 100 + '</td>',
      )
      return_table.cars_table_total_row.append(
        '<td>' + Math.round(sum_car_cond_length * 100) / 100 + '</td>',
      )
    } else {
      self.cars_lenght = 0
  ...
  this.check_car_number = function (p_car_number) {
    ...
    if (!find_result) {
      ...
      return_table.cars_table.find('tr td:contains("' + p_car_number + '")')
        .length !== 0
    ) {
      return_table.add_carnumber_input.addClass('red_bckg_color')
      alert('Вагон ' + p_car_number + ' уже добавлен!')
  ...
  this.add_cars_in_table = function (p_car_number, p_need_check) {
    ...
    if (
      ...
      $.ajax({
        ...
        dataType: 'text',
        async: false,
        data: { cars: add_car_number, ajax_action: 'get_add_info_for_cars' },
        success: function (data) {
          var records = JSON.parse(data)
          ...
          for (var i = 0; i < records.length; i++) {
            ...
            tr.append(
              $('<td>').append(
                $('<div>', { class: 'up_image' }).click(function () {
                  up_down_cars_table_tr('up', $(this).parent().parent())
                }),
              ),
            )
            tr.append(
              $('<td>').append(
                $('<div>', { class: 'down_image' }).click(function () {
                  up_down_cars_table_tr('down', $(this).parent().parent())
                }),
              ),
            )
            ...
            tr.append(
              '<td>' +
                (child.FREIGHT_NAME !== null ? child.FREIGHT_NAME : '') +
                '</td>',
            )
            tr.append(
              '<td>' +
                (child.WEIGHT_NET !== null
                  ? child.WEIGHT_NET.replace(',', '.')
                  : '') +
                '</td>',
            )
            tr.append(
              '<td>' +
                (child.WEIGHT_DEP !== null
                  ? child.WEIGHT_DEP.replace(',', '.')
                  : '') +
                '</td>',
            )
            tr.append(
              '<td>' +
                (child.WEIGHT_GROSS !== null
                  ? child.WEIGHT_GROSS.replace(',', '.')
                  : '') +
                '</td>',
            )
            tr.append(
              '<td>' +
                (child.CAR_LENGTH !== null
                  ? child.CAR_LENGTH.replace(',', '.')
                  : '') +
                '</td>',
            )
            tr.append(
              '<td>' +
                (child.COND_LENGTH_TRAIN !== null
                  ? child.COND_LENGTH_TRAIN.replace(',', '.')
                  : '') +
                '</td>',
            )
            tr.append(
              $('<td>').append(
                $('<div>', { class: 'deleteImage deleteImage13px' }).click(
                  function () {
...
function cont_table_const() {
  ...
  return_table.add_number_input = $('<input>', {
    type: 'text',
    css: { 'font-size': '11px' },
    class: 'text ui-widget-content ui-corner-all',
  })
  ...
  return_table.add_number_btn = $('<input>', {
    type: 'button',
    css: { 'font-size': '11px', height: '17px' },
    class: 'btnAdd',
  }).val('Добавить')
  ...
  return_table.table = $('<table>', { class: 'cont_table' }).append(
    $('<tbody>'),
  )
  return_table.append(
    $('<div>', {
  ...
  return_table.table_total_row = $('<tr>', {
    css: { background: '#EBEBEB', 'font-weight': 'bold' },
  })
  self.table_total_row = return_table.table_total_row
  return_table.append(
    $('<table>', { class: 'cont_table', css: { 'margin-top': '-4px' } }).append(
  ...
  function change_table_total_tr() {
    ...
    if (self.count !== 0) {
      ...
      return_table.table_total_row.append('<td></td>')
      return_table.table_total_row.append(
        '<td>' + Math.round(sum_weight_net * 100) / 100 + '</td>',
      )
      return_table.table_total_row.append(
        '<td>' + Math.round(sum_weight_dep * 100) / 100 + '</td>',
  ...
  this.add_in_table = function (p_number, p_need_check) {
    ...
    if (check_car_number_result && add_number !== null && add_number !== '') {
      $.ajax({
        ...
        dataType: 'text',
        async: false,
        data: { conts: add_number, ajax_action: 'get_add_info_for_conts' },
        success: function (data) {
          var records = JSON.parse(data)
          ...
          for (var i = 0; i < records.length; i++) {
            ...
            tr.append(
              $('<td>').append(
                $('<div>', { class: 'up_image' }).click(function () {
                  up_down_table_tr('up', $(this).parent().parent())
                }),
              ),
            )
            tr.append(
              $('<td>').append(
                $('<div>', { class: 'down_image' }).click(function () {
                  up_down_table_tr('down', $(this).parent().parent())
                }),
              ),
            )
...
function create_modal_dialog_move_inside_station() {
  function move_inside_station_ajax(
    p_cars,
    p_new_parent_id,
    p_new_parent_type,
    p_operation_date,
  ...
  function get_selected_cars_rows_for_moved_table() {
    ...
    l_cars.forEach(function (item) {
      res += '<tr>'
      res +=
        '<td><div class="up_image" onclick="up_down_moved_cars_table_tr(\'up\',$(this).parent().parent());"></div></td>'
      res +=
        '<td><div class="down_image" onclick="up_down_moved_cars_table_tr(\'down\',$(this).parent().parent());"></div></td>'
      res += '<td>' + j++ + '</td>'
      ...
      res +=
        '<td>' +
        '<div class="deleteImage deleteImage13px" title="Удалить" onclick="$(this).parent().parent().remove(); change_moved_cars_table_total_tr();md_disable_moved_btn();"></div>' +
        '</td>'
      res += '</tr>'
  ...
  function get_select_with_station_child(p_id, p_station_id) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { station_id: p_station_id, ajax_action: 'get_all_station_child' },
      success: function (data) {
        var records = JSON.parse(data)
        $.each(records, function (i, item) {
          result +=
            '<option ' +
            (item.DISABLED == 'Y' ? 'disabled' : '') +
            ' style="' +
            ('margin-left: ' + (item.LVL - 1) * 10 + 'px') +
            '" data-id="' +
            item.ID +
            '" data-type="' +
            item.TYPE +
  ...
  function get_moved_cars() {
    ...
    $('table#moved_cars_table tbody tr').each(function () {
      param +=
        $(this).children('td:nth-child(4)').text() + '|' + 'railcar' + '$'
    })
  ...
  $('<div/>')
    ...
    .append(
      '<div style="display: table;">' +
        ...
        '<input disabled type="text" size="13" class="text ui-widget-content ui-corner-all" value="' +
        user_station_name +
        '"></input>' +
        '</div>' +
        '</div>' +
        '<div class="attr" style="margin-left:14px; text-align:right; float:right">' +
        '<input disabled type="text" size="14" class="text ui-widget-content ui-corner-all" value="' +
        server_current_time +
        '"></input><br>' +
        '<input disabled type="text" size="20" class="text ui-widget-content ui-corner-all" style="margin-top:10px;" value="' +
        user_name +
        '"></input>' +
        '</div>' +
        '</div>' +
        ...
        get_select_with_station_child(
          'md_new_parent',
          $('#currentCarstree li[data-type="station"]').attr('data-id'),
        ) +
        '</div>' +
        '</div>' +
        '</div>' +
  ...
  $('#md_operation_date').blur(function () {
  ...
  var in_station_cars = $(
    '#currentCarstree > li[data-type="station"][data-id="' +
      user_station_id +
      '"] li[data-type="railcar"]',
  )
  ...
  $('#md_moved_add_car').autocomplete({
    source: in_station_cars_mas,
    minLength: 2,
  })
        ...
        click: function () {
          if (check_open_period('2', $('#md_operation_date').val()) == '0') {
            create_info_modal_dialog_new(
              'Оповещение',
              'Для даты ' +
                $('#md_operation_date').val() +
                ' нет открытого периода! Переместить вагоны не возможно!',
            )
          } else {
            ...
            var new_parent = {
              id: $('#md_new_parent > option:selected').attr('data-id'),
              type: $('#md_new_parent > option:selected').attr('data-type'),
            }

            if (
              move_inside_station_ajax(
                l_cars,
                new_parent.id,
                new_parent.type,
                $('#md_operation_date').val(),
              ...
              create_info_modal_dialog_new(
                'Ошибка',
                'Процедура завершилась с ошибкой!',
              )
            }
...
function add_car_in_moved_table() {
  ...
  if (
    ...
    sent_table.find('tr td:contains("' + add_car + '")').length !== 0
  ) {
    $('#md_moved_add_car').addClass('red_bckg_color')
...
function md_disable_moved_btn() {
  if (
    $('#md_operation_date').hasClass('red_bckg_color') ||
    $('#md_operation_date').val() == '' ||
    $('#md_new_parent').val() == '' ||
    $('#moved_cars_table tbody tr').length === 0
  ) {
...
function md_move_inside_station_alert() {
  if ($('#md_new_parent').val() == '' || $('#md_new_parent').val() == '-1') {
    ...
    var free_cars_count = parseInt(
      $('#md_new_parent option:selected').attr('data-cars_count'),
    )
    var free_cars_length = parseInt(
      $('#md_new_parent option:selected').attr('data-free_length'),
    )

    var cars_count = parseInt(
      $('#moved_cars_table_total_tr td:nth-child(4)').text().match(/[^ ]+/g)[1],
    )

    var cars_length = parseFloat(
      $('#moved_cars_table_total_tr td:nth-child(9)').text(),
    )
    ...
    if (cars_count > free_cars_count) {
      ...
      td_count
        ...
        .attr(
          'title',
          'Кол-во вагонов превысило допустимое значение: ' +
            cars_count +
            ' > ' +
            free_cars_count,
        )
      ...
      td_count.removeClass('red_bckg_color').attr('title', '')
      if (td_count.tooltip('instance') != null) {
        td_count.tooltip('disable')
      }
    }
    ...
    if (cars_length > free_cars_length) {
      ...
      td_length
        ...
        .attr(
          'title',
          'Длина состава превысила допустимое значение: ' +
            cars_length +
            ' > ' +
            free_cars_length,
        )
        ...
        .tooltip('open')
      setTimeout(function () {
        td_length.tooltip('close')
      }, 2500)
    } else {
      var td_length = $('#moved_cars_table_total_tr td:nth-child(9)')
      td_length.removeClass('red_bckg_color').attr('title', '')
      if (td_length.tooltip('instance') != null) {
        td_length.tooltip('disable')
      }
    }
  }
}
...
function create_modal_dialog_send_to_station(event) {
  ...
  function get_selected_cars_rows() {
    ...
    l_cars.forEach(function (item) {
      res += '<tr>'
      res +=
        '<td><div class="up_image" onclick="up_down_sent_cars_table_tr(\'up\',$(this).parent().parent());"></div></td>'
      res +=
        '<td><div class="down_image" onclick="up_down_sent_cars_table_tr(\'down\',$(this).parent().parent());"></div></td>'
      res += '<td>' + j++ + '</td>'
      res += '<td>' + item.car_number + '</td>'
      res += '<td>' + item.freight + '</td>'
      res += '<td>' + item.weight_net + '</td>'
      ...
      res +=
        '<td>' +
        '<div class="deleteImage deleteImage13px" title="Удалить" onclick="del_sent_cars_table_tr($(this).parent().parent()); change_sent_cars_table_total_tr(); md_disable_send_btn();"></div>' +
        '</td>'
      res += '</tr>'
      //res+= $(this).children('td:nth-child(1)').text() + '|' + 'railcar' + '$';
    })
  ...
  function run_report(p_type) {
    ...
    var l_loco1_driver1 = $(
      '#md_send_to_station_loco1_driver1 option:selected',
    ).text()
    var l_loco1_driver2 = $(
      '#md_send_to_station_loco1_driver2 option:selected',
    ).text()
    var l_loco1_conductor = $(
      '#md_send_to_station_loco1_conductor option:selected',
    ).text()
    var l_loco2_num = $('#md_send_to_station_loco2_num option:selected').text()
    var l_loco2_driver1 = $(
      '#md_send_to_station_loco2_driver1 option:selected',
    ).text()
    var l_loco2_driver2 = $(
      '#md_send_to_station_loco2_driver2 option:selected',
    ).text()
    var l_loco2_conductor = $(
      '#md_send_to_station_loco2_conductor option:selected',
    ).text()
    var l_cars = get_sent_cars_for_wagon_list()

    var win = window.open(
      'wagon_list/wagon_list_word.php?' +
        'date_from=' +
        $('#md_send_to_station_sending_time').val() +
        '&' +
        'report=' +
        p_type +
        '&' +
  ...
  $('<div/>')
    ...
    .append(
      '<div style="display: table;">' +
        ...
        '<input id="md_save_send_to_station_form_inp" class="md_save_load" type="text" value="' +
        user_name.slice(0, user_name.indexOf(' ')) +
        '_' +
        server_current_time +
        '" maxlength="30">' +
        '</div>' +
  ...
  $('#md_send_to_station_loco1_num, #md_send_to_station_loco2_num').on(
    'change',
    function () {
  ...
  $('#md_send_to_station_add_car').autocomplete({
    source: in_station_cars_mas,
    minLength: 2,
  })
  /*end заполняем input с автозаполнением вагонов*/
  ...
  init_date_time_input_btw(
    $('#md_send_to_station_sending_time'),
    l_compare_date_from,
    l_compare_date_to,
  )
  ...
  $('#md_send_to_station_sending_time,#md_send_to_station_arrival_time').on(
    'blur',
    function (e) {
      ...
      var l_sending_time_d = new Date(
        l_sending_time.replace(pattern, '$3-$2-$1T$4:$5:00'),
      )
      var l_arrival_time_d = new Date(
        l_arrival_time.replace(pattern, '$3-$2-$1T$4:$5:00'),
      )

      var diff_hh = parseInt(
        (l_arrival_time_d - l_sending_time_d) / 1000 / 60 / 60,
      )
      var diff_mi = ((l_arrival_time_d - l_sending_time_d) / 1000 / 60) % 60

      $('#md_send_to_station_estimated_time').val(
        (diff_hh < 0 || diff_mi < 0 ? '-' : '') +
          Math.abs(diff_hh) +
          ':' +
          Math.abs(diff_mi),
  ...
  $(
    '#md_send_to_station_sending_time,#md_send_to_station_arrival_time,#md_send_to_station_train_num',
  ).blur(function () {
    md_disable_send_btn()
  })
        ...
        click: function () {
          if (
            ...
            create_info_modal_dialog_new(
              ...
              'Для даты ' +
                $('#md_send_to_station_sending_time').val() +
                ' нет открытого периода! Отправить вагоны с данной датой не возможно!',
            )
          } else if (
            (user_station_id == '3' || target.attr('data-id') == '3') &&
            check_open_period(
              '2',
              $('#md_send_to_station_sending_time').val(),
            ) == '0'
            ...
            if (
              send_to_station_ajax(
                l_cars,
                user_station_id,
                target.attr('data-id'),
                $('#md_send_to_station_sending_time').val(),
                $('#md_send_to_station_arrival_time').val(),
                $('#md_send_to_station_reason').val(),
                $('#md_send_to_station_train_num').val(),
                $('#md_send_to_station_loco1_num').val(),
                $('#md_send_to_station_loco1_driver1').val(),
                $('#md_send_to_station_loco1_driver2').val(),
                $('#md_send_to_station_loco1_conductor').val(),
                $('#md_send_to_station_loco2_num').val(),
                $('#md_send_to_station_loco2_driver1').val(),
                $('#md_send_to_station_loco2_driver2').val(),
                $('#md_send_to_station_loco2_conductor').val(),
  ...
  if (
    ...
    $(
      '#currentCarstree > li[data-type="station"][data-id="' +
        ...
        '"] li[data-id="' +
        add_car +
        '"][data-type="railcar"][data-notification-gu="Y"]',
    ).length == 0
  ) {
...
function md_disable_send_btn() {
  if (
    $('#md_send_to_station_sending_time').hasClass('red_bckg_color') ||
    $('#md_send_to_station_sending_time').val() == '' ||
    $('#md_send_to_station_arrival_time').hasClass('red_bckg_color') ||
    $('#md_send_to_station_arrival_time').val() == '' ||
    $('#md_send_to_station_train_num').val() == '' ||
    $('#sent_cars_table tbody tr').length === 0
  ) {
...
function md_send_to_station_alert() {
  ...
  var loco_count =
    ($('#md_send_to_station_loco1_num').val() == '-1' ? 0 : 1) +
    ($('#md_send_to_station_loco2_num').val() == '-1' ? 0 : 1)
  ...
  var cars_length = parseFloat(
    $('#sent_cars_table_total_tr td:nth-child(9)').text(),
  )
  var cars_weight = parseFloat(
    $('#sent_cars_table_total_tr td:nth-child(8)').text(),
  )
  ...
  if (cars_weight > max_railway_weight) {
    ...
    td_weight
      ...
      .attr(
        'title',
        'Масса состава превысила допустимое значение: ' +
          cars_weight +
          ' > ' +
          max_railway_weight,
      )
...
function get_railway_parts(p_railway_id, p_part_id) {
  ...
  $.ajax({
    ...
    async: false,
    data: {
      params: JSON.stringify(l_params),
...
function get_railway_add_info(p_railway_id) {
  ...
  $.ajax({
    ...
    dataType: 'text',
    async: false,
    data: { railway_id: p_railway_id, ajax_action: 'get_railway_add_info' },
    success: function (data) {
      records = JSON.parse(data)
    },
  })
...
function save_send_to_station_form_btn_action() {
  ...
  $(
    '#md_save_send_to_station_form_btn,#md_load_send_to_station_form_btn,#md_del_send_to_station_form_btn',
  ).removeClass('md_save_load_btn_done md_save_load_btn_fail')

  if (save_send_to_station_form_ajax() === 'done') {
    $('#md_save_send_to_station_form_btn').addClass('md_save_load_btn_done')
    $(
      '#md_load_send_to_station_form_sel,#md_del_send_to_station_form_sel',
    ).empty()
    $(
      '#md_load_send_to_station_form_sel,#md_del_send_to_station_form_sel',
    ).append(get_select_options_with_saved_send_to_station_forms())
  } else {
...
function load_send_to_station_form_btn_action() {
  ...
  $.ajax({
    ...
    async: false,
    data: {
      name: $('#md_load_send_to_station_form_sel').val(),
    ...
    success: function (data) {
      try {
        ...
        $('#md_save_send_to_station_form_inp').val(
          $('#md_load_send_to_station_form_sel').val(),
        )
...
function createContectMenu2lvl(addId, p_x, p_y) {
  if ($('#context-menu-2lvl' + addId).length === 0) {
    $.ajax({
      ...
      async: false,
      data: {
        station_id: $('#selectStation option:selected').val(),
...
function createModalDialogAddBandwagon() {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data !== '0') {
          ...
          li.append(
            '<div class="tree_Expand"></div><div class="tree_Content">' +
              $('#modalDialogAddName').val() +
              '</div>',
          )
  ...
  $('<div/>')
    ...
    .appendTo('body') // Присоединяем наше меню к body документа:
    .append(
      '<label for="modalDialogAddName">Наименование</label><br>' +
...
function createModalDialogDeleteBandwagon() {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data === 'done') {
          $(
            'li[data-id="' +
              clickedLi.attr('data-id') +
              '"][data-type="' +
              clickedLi.attr('data-type') +
              '"]',
          ).remove()
        } else {
  ...
  if (clickedLi.find('.tree_Container > li').length === 0) {
    $('<div/>')
      ...
      .appendTo('body') // Присоединяем наше меню к body документа:
      .append(
        '<p>Вы хотите удалить группу вагонов: "' +
          clickedLi.children('.tree_Content').text() +
          '"? </p>',
      )
    dialog = $('#modalDialog').dialog({
      resizable: false,
    ...
    $('<div/>')
      ...
      .appendTo('body') // Присоединяем наше меню к body документа:
      .append(
        '<p>Удаление группы вагонов: "' +
          clickedLi.children('.tree_Content').text() +
          '" не возможно, т.к. существуют дочерние элементы!</p>',
      )
    dialog = $('#modalDialog').dialog({
      resizable: false,
      modal: true,
...
function addInfo(data, ctrlKey) {
  ...
  for (var i = 0; i < records.length; i++) {
    var child = records[i]
    if (
      $('table.addInfoTable tbody tr td:contains("' + child.ID + '")')
        .length === 0
    ) {
      var ClassFromUgl
      ...
      tr.append(
        '<td><div>' +
          (child.SCALE_WEIGHT_DEP !== null
            ? child.SCALE_WEIGHT_DEP.replace(',', '.')
            : '') +
          '</div>\
                           <div>' +
          (child.SCALE_WEIGHT_GROSS !== null
            ? child.SCALE_WEIGHT_GROSS.replace(',', '.')
            : '') +
          '</div></td>',
      )
      tr.append(
        '<td class ="' +
          ClassFromUgl +
          '">' +
          (child.INV_NUMBER !== null ? child.INV_NUMBER : '') +
          '</td>',
      )
      tr.append(
        '<td>' +
          (child.CONT_WITH_INS !== null ? child.CONT_WITH_INS : '') +
          '</td>',
      )
      tr.append('<td>' + (child.OWNER !== null ? child.OWNER : '') + '</td>')
      tr.append(
        '<td>' +
          (child.DATE_ARRIVE !== null ? child.DATE_ARRIVE : '') +
          '</td>',
      )
      tr.append(
        '<td>' +
          (child.DATE_LAST_OPER !== null ? child.DATE_LAST_OPER : '') +
          '</td>',
      )
      tr.append(
        '<td>' +
          (child.DATE_LOADING !== null ? child.DATE_LOADING : '') +
          '</td>',
      )
      tr.append(
      ...
      tr.append(
        '<td style="display: none;">' +
          (child.ARRIVE_WEIGHT_NET !== null
            ? child.ARRIVE_WEIGHT_NET.replace(',', '.')
            : '') +
          '</td>',
      )
      tr.append(
        '<td style="display: none;">' +
          (child.EXISTS_POST !== null ? child.EXISTS_POST : 'N') +
          '</td>',
      )
      tr.append(
        '<td style="display: none;">' +
          (child.CONT !== null ? child.CONT : '') +
          '</td>',
      )
      tr.append(
        '<td style="display: none;">' +
          (child.OBJ_TYPE !== null ? child.OBJ_TYPE : '') +
          '</td>',
      )
      tr.append(
        '<td style="display: none;">' +
          (child.CAN_CREATE_RETURN_INVOICE !== null
            ? child.CAN_CREATE_RETURN_INVOICE
            : '') +
          '</td>',
      )
      tr.append(
        '<td style="display: none;">' +
          (child.RAILCAR_TYPE !== null ? child.RAILCAR_TYPE : '') +
          '</td>',
      )
      tr.append(
        '<td style="display: none;">' +
          (child.UN_LOADING_SUBS !== null ? child.UN_LOADING_SUBS : '') +
          '</td>',
      )
      tr.append(
        '<td style="display: none;">' +
          (child.COND_LENGTH_TRAIN !== null ? child.COND_LENGTH_TRAIN : '') +
          '</td>',
      )

      if (
        child.SCALE_WEIGHT_DEP !== null &&
        child.SCALE_WEIGHT_DEP_ADD == 'Y'
      ) {
...
function remAddInfo(clickedElem) {
  if (
    clickedElem.attr('data-type') === 'railcar' ||
    clickedElem.attr('data-type') === 'cont'
  ) {
    $(
      'table.addInfoTable tbody tr td:contains("' +
        clickedElem.attr('data-id') +
        '")',
    )
      .parent('tr')
      .remove()
...
function changeTotalTr() {
  ...
  if ($('table.addInfoTable tbody tr').length !== 0) {
    ...
    tr.append(
      '<td>Кол-во: ' + $('table.addInfoTable tbody tr').length + '</td>',
    )
    tr.append('<td></td>')
    tr.append('<td></td>')
    tr.append('<td></td>')
...
function create_md_send_from_ugl() {
  function checkRequired() {
    if (getIsEmpty($('#modalDialogDateFact').val())) {
      create_info_modal_dialog_new(
        'Оповещение',
        'Не заполнено поле "Дата и время операции (мск)"!',
      )
      return false
    }
    ...
    if (getIsEmpty($('#modalDialogComment').val())) {
      create_info_modal_dialog_new(
        'Оповещение',
        'Не заполнено поле "Комментарий (указать накладную)"!',
      )
      return false
    }
    return true
  ...
  var md_content = $('<div/>')
    ...
    .appendTo('body') // Присоединяем наше меню к body документа:
    .append(
      '<label for="modalDialogDateFact">Дата и время операции (мск)</label><br>' +
        ...
        if (!checkRequired()) {
          ...
          if (sendFromUglAjax(param) === 'done') {
            params.forEach(function (item, i, arr) {
              var items = item.split('|')
              $(
                'li[data-id="' + items[0] + '"][data-type="' + items[1] + '"]',
              ).remove()
            })
            changeRailcarCount()
...
function entry_foreign_railcar() {
  ...
  $('#modalDialogWeightNet,#modalDialogWeightDep').on('keyup', function (e) {
    if (
      !isNaN(+$('#modalDialogWeightNet').val()) &&
      !isNaN(+$('#modalDialogWeightDep').val())
    ) {
      $('#modalDialogWeightGross').val(
        parseFloat(+$('#modalDialogWeightNet').val()) +
          parseFloat(+$('#modalDialogWeightDep').val()),
      )
    }
  })
  ...
  $('.entryRailcarTable tbody').on('click', 'tr', function (e) {
    ...
    $('#modalDialogCarNumber').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(1)').text(),
    )
    $('#modalDialogCarType').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(2)').text(),
    )
    $('#modalDialogStatus').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(3)').text(),
    )
    $('#modalDialogState').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(4)').text(),
    )
    $('#modalDialogFreightName').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(5)').text(),
    )
    $('#modalDialogWeightNet').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(6)').text(),
    )
    $('#modalDialogWeightDep').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(7)').text(),
    )
    $('#modalDialogWeightGross').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(8)').text(),
    )
    $('#modalDialogInvNumber').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(9)').text(),
    )
    $('#modalDialogCont').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(10)').text(),
    )
    $('#modalDialogOwner').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(11)').text(),
    )
    $('#modalDialogDateArrive').val(
      $('.entryRailcarTable tbody tr.selected td:nth-child(12)').text(),
    )
...
function entry_enemy_railcar_chg_tr() {
  ...
  var l_exists = md_entry_foreign_railcar_exists_carnumber_in_table(
    l_car_number,
    true,
  )
...
function md_entry_foreign_railcar_exists_carnumber_in_table(
  p_car_number,
  p_not_selected,
) {
  var exists = false
  if (p_car_number !== '') {
    $(
      '.entryRailcarTable tbody tr:not(' +
        (p_not_selected ? '.selected' : '') +
        ') td:nth-child(1)',
    ).each(function () {
...
function md_entry_foreign_railcar_disable_add_btn() {
  ...
  if (
    !$('#modalDialogCarNumber').hasClass('true-car-number') ||
    $('#modalDialogInvNumber').val() == '' ||
    $('#modalDialogDateArrive').hasClass('red_bckg_color') ||
    $('#modalDialogDateArrive').val() == '' ||
    $('#modalDialogOwner').val() == '' ||
    l_exists
  ) {
...
function create_md_received_into_station_new() {
  function md_received_into_station_constr() {
        ...
        $(
          ...
          p_select.append(
            '<option selected data-id="' +
              ...
              $(this).attr('data-id') +
              '">' +
              $(this).children('.tree_Content').text() +
              '</option>',
          )
        })
      }
    ...
    this.show_window = function () {
      ...
      var md_div = $('<div/>')
        ...
        .append(
          '<div style="display: table;">' +
            ...
            '"></input><br>' +
            '<input disabled type="text" size="20" class="text ui-widget-content ui-corner-all" style="margin-top:5px;" value="' +
            user_name +
            '"></input>' +
            '</div>' +
            '</div>',
      ...
      md_div.operation_date = $('<input>', {
        type: 'text',
        class: 'text ui-widget-content ui-corner-all required',
      }).attr('size', '15')
      md_div.comment = $('<input>', {
        type: 'text',
        class: 'text ui-widget-content ui-corner-all',
        css: { width: '350px' },
      })
      ...
      md_div.railway = get_select_with_station_child(
        $('#currentCarstree li[data-type="station"]').attr('data-id'),
      )
      md_div.bef_after = $('<select>').append(
      ...
      md_div.append(
        $('<div>', { css: { display: 'table' } }).append(
          $('<div>', { class: 'border', css: { clear: 'both' } })
            .append(
              $('<div>', { class: 'header', css: { width: '180px' } }).text(
                'Cтанция назначения',
              ),
            )
            .append(
      ...
      md_div.wagon_list_div = $('<div>', {
        class: 'border',
        css: { clear: 'both' },
      })
      md_div.train_num = $('<input>', {
        type: 'text',
        class: 'text ui-widget-content ui-corner-all required',
      }).attr('size', '4')
      ...
      md_div.append(
        md_div.wagon_list_div
          .append(
            $('<div>', { class: 'header', css: { width: '130px' } }).text(
              'Натурный лист',
            ),
          )
          .append(
      ...
      var check_need_wagon_list = function (p_car_number) {
        if ($('#currentCarstree > li').attr('data-id') != '2') {
          ...
          $(
            'li[data-id="' +
              p_car_number +
              '"][data-type="railcar"][data-from_station_id="1"][data-notification="Y"]',
          ).length == 0
        ) {
          md_div.wagon_list_div.hide()
        }
      ...
      md_div.railway.on('select', function () {
        fill_railcars_for_railway(
          md_div.bef_after_railcar,
          md_div.railway.children('option:selected').attr('data-id'),
          md_div.railway.children('option:selected').attr('data-type'),
        )
      })
      ...
      $.ajax({
        ...
        dataType: 'text',
        async: false,
        data: { station_id: user_station_id, ajax_action: 'get_coming_cars' },
        success: function (data) {
      ...
      railcar_table.car_number_input.autocomplete({
        source: coming_cars_mas,
        minLength: 2,
      })
      ...
      md_div.disable_ok_btn = function () {
        if (
          md_div.operation_date.hasClass('red_bckg_color') ||
          md_div.operation_date.val() == '' ||
          md_div.railway.val() == '' ||
          railcar_table.cars_count == 0 ||
          (md_div.wagon_list_div.is(':visible') &&
            (md_div.sending_time.hasClass('red_bckg_color') ||
              md_div.sending_time.val() == '' ||
              md_div.train_num.val() == ''))
        ) {
      ...
      md_div.check_total_tr = function () {
        if (md_div.railway.val() == '' || md_div.railway.val() == '-1') {
          ...
          var free_cars_count = parseInt(
            md_div.railway.children('option:selected').attr('data-cars_count'),
          )
          var free_cars_length = parseInt(
            md_div.railway.children('option:selected').attr('data-free_length'),
          )
          ...
          if (cars_count > free_cars_count) {
            var td_count =
              railcar_table.cars_table_total_row.children('td:nth-child(4)') //$('#received_cars_table_total_tr td:nth-child(4)');
            td_count
              .addClass('red_bckg_color')
              .attr(
                'title',
          ...
          if (cars_length > free_cars_length) {
            var td_length =
              railcar_table.cars_table_total_row.children('td:nth-child(9)') //$('#received_cars_table_total_tr td:nth-child(9)');
            td_length
              .addClass('red_bckg_color')
              .attr(
                'title',
            ...
            click: function () {
              if (check_open_period('2', md_div.operation_date.val()) == '0') {
                create_info_modal_dialog_new(
                  ...
                  'Для даты ' +
                    md_div.operation_date.val() +
                    ' нет открытого периода! Принять вагоны не возможно!',
                )
              } else {
                received_into_station_OK(
                  railcar_table.get_cars_in_table(),
                  md_div.railway.children('option:selected').attr('data-id'),
                  md_div.railway.children('option:selected').attr('data-type'),
                  md_div.bef_after.children('option:selected').val(),
                  md_div.bef_after_railcar
                  ...
                  md_div.sending_time.val(),
                  md_div.train_num.val(),
                  md_div.loco1_num.val(),
                  md_div.loco1_driver1.val(),
                  md_div.loco1_driver2.val(),
                  md_div.loco1_conductor.val(),
                  md_div.loco2_num.val(),
                  md_div.loco2_driver1.val(),
                  md_div.loco2_driver2.val(),
                  md_div.loco2_conductor.val(),
                )
                $(this).dialog('close')
              }
            },
...
function received_into_station_OK(
  p_cars,
  p_new_parent_id,
  p_new_parent_type,
  p_bef_aft,
  ...
  function ReceivedIntoStationAjax(
    p_elem,
    p_new_parent_id,
    p_new_parent_type,
    p_after_elem_id,
  ...
  var after_elem_is_last =
    p_after_elem_id === undefined ||
    p_after_elem_type === undefined ||
    after_elem.is(':last-child')
  if (p_bef_aft === 'after') {
...
function create_modal_dialog_fill_railcar_attr(p_clicked_li) {
  ...
  function get_select_freight_list() {
    var l_freight_name = $(
      'table.addInfoTable > tbody > tr:first-child > td:nth-child(5)',
    ).text()

    var result =
      '<select id="modalDialogFreightName" ' +
      (l_freight_name === '' ? '' : 'disabled') +
      '>'
  ...
  function get_select_car_type_list() {
    var l_car_type = $(
      'table.addInfoTable > tbody > tr:first-child > td:nth-child(2)',
    ).text()

    var result =
      '<select id="modalDialogCarType" ' +
      (l_car_type === '' ? '' : 'disabled') +
      '>'
    ...
    buttons: {
      Добавить: function () {
        if (
          ...
          $(
            'table.addInfoTable > tbody > tr:first-child > td:nth-child(2)',
          ).text($('#modalDialogCarType').val())
          $(
            'table.addInfoTable > tbody > tr:first-child > td:nth-child(5)',
          ).text($('#modalDialogFreightName').val())
    ...
    var md = $('<div/>')
      ...
      .append(
        '<div>' +
          ...
          createSelectBadInvoices('md_invoice') +
          '<br>' +
    ...
    md.dialog({
      ...
      buttons: {
        Добавить: {
          ...
          click: function () {
            if (
              fill_railcar_for_invoice_ajax(
                $('#md_invoice option:selected').val(),
                $('#md_invoice_car_number').val(),
              ) === 'done'
            ) {
              changeBackground()
              create_info_modal_dialog_new(
...
function create_md_change_cont_attr(p_clicked_li, p_cont_number) {
  function get_freight_select(p_selected_elem) {
    ...
    $('<option>').val(p_selected_elem).text(p_selected_elem).appendTo(select)
  ...
  $.ajax({
    ...
    dataType: 'text',
    async: false,
    data: { car_number: p_cont_number, ajax_action: 'get_add_info_for_cont' },
    success: function (data) {
      var records = JSON.parse(data)
      var child = records[0]
      l_cont_info.freight =
      ...
      l_cont_info.weight_dep =
        child.WEIGHT_DEP !== null ? child.WEIGHT_DEP.replace(',', '.') : ''
    },
    error: function (m1, m2) {
      window.alert(m1 + m2)
    },
      ...
      Изменить: {
        ...
        click: function () {
          ...
          if (res === 'done') {
            ...
            create_info_modal_dialog_new(
              'Предупреждение',
              'Изменить данные не возможно, т.к. есть уже накладная на отправку!',
            )
          } else {
            create_info_modal_dialog_new(
              'Ошибка',
...
function create_md_change_attr(p_clicked_li) {
  ...
  function get_car_type_select(p_selected_elem) {
    var select = $('<select>')
    $.each(g_car_type_list, function (i, item) {
      $('<option>').val(item.CAR_TYPE).text(item.CAR_TYPE).appendTo(select)
    })
    $('<option>').val(p_selected_elem).text(p_selected_elem).appendTo(select)
...
function create_md_change_attr_old(p_clicked_li) {
  ...
  $('<div/>')
    ...
    .append(
      '<div class="attr">' +
        '<div>' +
        '<label for="modalDialogCarType">Тип</label>' +
        '<input disabled id="modalDialogCarType" type="text" maxlength="20" size="6" class="text ui-widget-content ui-corner-all" value="' +
        $(
          'table.addInfoTable > tbody > tr:first-child > td:nth-child(2)',
        ).text() +
        '"><br>' +
        ...
        '<input disabled id="modalDialogWeightDep" type="text" maxlength="50" size="5" class="text ui-widget-content ui-corner-all" value="' +
        $(
          'table.addInfoTable > tbody > tr:first-child > td:nth-child(7)',
        ).text() +
        '"><br>' +
        '</div>' +
        '<div>' +
        '<label for="modalDialogCont">№ контейнера</label>' +
        '<input disabled id="modalDialogCont" type="text" maxlength="50" size="25" class="text ui-widget-content ui-corner-all" value="' +
        $(
          'table.addInfoTable > tbody > tr:first-child > td:nth-child(10)',
        ).text() +
        '"><br>' +
        '</div>' +
        '<div>' +
        '<label for="modalDialogOwner">Предприятие</label>' +
        '<input disabled id="modalDialogOwner" type="text" maxlength="3" size="3" class="text ui-widget-content ui-corner-all" value="' +
        $(
          'table.addInfoTable > tbody > tr:first-child > td:nth-child(11)',
        ).text() +
        '"><br>' +
        '</div>' +
        '</div>',
  ...
  $('#modalDialogDateLoading').blur(function () {
...
function md_disable_change_attr_btn() {
  if (
    $('#modalDialogDateLoading').hasClass('red_bckg_color') ||
    $('#modalDialogDateLoading').val() == '' ||
    isNaN(+$('#modalDialogWeightNet').val()) ||
    isNaN(+$('#modalDialogWeightDep').val())
  ) {
...
function create_modal_dialog_notification() {
  ...
  function get_selected_cars_rows_for_notification_table() {
    ...
    $('table.addInfoTable tbody tr:not(:last)').each(function () {
      res += '<tr>'
      res +=
        '<td><div class="up_image" onclick="up_down_notifications_cars_table_tr(\'up\',$(this).parent().parent());"></div></td>'
      res +=
        '<td><div class="down_image" onclick="up_down_notifications_cars_table_tr(\'down\',$(this).parent().parent());"></div></td>'
      res += '<td>' + j++ + '</td>'
      ...
      res +=
        '<td>' +
        '<div class="deleteImage deleteImage13px" title="Удалить" onclick="$(this).parent().parent().remove(); change_notification_cars_table_total_tr();md_disable_notification_btn();"></div>' +
        '</td>'
      res += '</tr>'
      //res+= $(this).children('td:nth-child(1)').text() + '|' + 'railcar' + '$';
    })
  ...
  $('<div/>')
    ...
    .append(
      '<div style="display: table;">' +
        ...
        '<input id="md_save_notification_inp" class="md_save_load" type="text" value="' +
        user_name.slice(0, user_name.indexOf(' ')) +
        '_' +
        server_current_time +
        '" maxlength="30">' +
  ...
  $('#md_notification_add_car').autocomplete({
    source: g_cars_in_ugl_mas,
    minLength: 2,
  })
  /*end заполняем input с автозаполнением вагонов*/
  ...
  $(
    '#md_notification_time_from,#md_notification_person_from,#md_notification_time_to',
  ).blur(function () {
    md_disable_notification_btn()
  })
  $('#md_notification_person_to').change(function () {
    md_disable_notification_btn()
  })
  ...
  $('#modalDialog').dialog({
    ...
    buttons: {
      Отправить: {
        ...
        click: function () {
          if (
            check_open_period('2', $('#md_notification_time_from').val()) == '0'
          ) {
            create_info_modal_dialog_new(
              'Оповещение',
              'Для даты ' +
                $('#md_notification_time_from').val() +
                ' нет открытого периода! Сохранить уведомление не возможно!',
            )
          } else {
            ...
            if (
              notification_ajax(
                l_cars,
                $('#md_notification_time_from').val(),
                $('#md_notification_person_from').val(),
                $('#md_notification_railway_number').val(),
                $('#md_notification_time_to').val(),
                $('#md_notification_person_to').val(),
                $('#md_save_notification_inp').val(),
              ) === 'done'
            ) {
              l_cars_mas.forEach(function (item, i, arr) {
                var items = item.split('|')
...
function md_save_notification_btn_action() {
  function save_notification_ajax(
    p_notification_name,
    p_cars,
    p_notification_time_from,
    p_notification_person_from,
  ...
  $(
    '#md_save_notification_btn,#md_load_notification_btn,#md_del_notification_btn',
  ).removeClass('md_save_load_btn_done md_save_load_btn_fail')
  var l_cars = get_cars_for_notification() //сохраняем выбранные элементы
  if (
    save_notification_ajax(
      $('#md_save_notification_inp').val(),
      l_cars,
      $('#md_notification_time_from').val(),
      $('#md_notification_person_from').val(),
    ...
    $('#md_load_notification_sel,#md_del_notification_sel').append(
      get_select_options_with_saved_notifications(),
    )
  } else {
...
function md_load_notification_btn_action() {
  ...
  $.ajax({
    ...
    async: false,
    data: {
      notification_name: $('#md_load_notification_sel').val(),
    ...
    success: function (data) {
      try {
        ...
        $('#md_notification_time_from').val(
          l_notification[0].NOTIFICATION_TIME_FROM,
        )
        $('#md_notification_person_from').val(
          l_notification[0].NOTIFICATION_PERSON_FROM,
        )
        $('#md_notification_railway_number').val(
          l_notification[0].NOTIFICATION_RAILWAY_NUMBER,
        )
        $('#md_notification_time_to').val(
          l_notification[0].NOTIFICATION_TIME_TO,
        )
        $('#md_notification_person_to').val(
          l_notification[0].NOTIFICATION_PERSON_TO,
        )
...
function add_car_in_notification_table(p_railcar) {
  ...
  if (g_cars_in_ugl_mas.indexOf(add_car) === -1) {
    ...
    alert(
      'На путях нет вагона с номером ' +
        add_car +
        ' или у него есть уже уведомление!',
    )
  } else if (
    notification_table.find('tr td:contains("' + add_car + '")').length !== 0
  ) {
    $('#md_notification_add_car').addClass('red_bckg_color')
    alert('Вагон ' + add_car + ' уже добавлен!')
  } else {
...
function change_notification_cars_table_total_tr() {
  ...
  if ($('#notification_cars_table tbody tr').length !== 0) {
    ...
    tr.append('<td></td>')
    tr.append(
      '<td>Кол-во: ' + $('#notification_cars_table tbody tr').length + '</td>',
    )
    tr.append('<td></td>')
    tr.append('<td>' + Math.round(sum_weight_net * 100) / 100 + '</td>')
    tr.append('<td>' + Math.round(sum_weight_dep * 100) / 100 + '</td>')
...
function md_disable_notification_btn() {
  if (
    $('#md_notification_time_from').hasClass('red_bckg_color') ||
    $('#md_notification_time_from').val() == '' ||
    $('#md_notification_person_from').val() == '' ||
    $('#md_notification_time_to').hasClass('red_bckg_color') ||
    $('#md_notification_time_to').val() == '' ||
    $('#md_notification_person_to').val() == '' ||
...
function create_md_inspections(p_clicked_li) {
  ...
  function md_enter_inspection_ajax(
    p_car_number,
    p_cont,
    p_inspection_id,
    p_inspection_date,
  ...
  var md_ins_div = $('<div/>')
    ...
    .append(
      '<div id="md_ins_refresh_railcar_div" class="border" style="display: table;">' +
        ...
        '<div>' +
        '<label for="md_ins_inspection_created_users">Создал</label>' +
        '<input id="md_ins_inspection_created_users" type="text" maxlength="400" style="width: 30em;" class="text ui-widget-content ui-corner-all" readonly><br>' +
        '</div>' +
        '<div>' +
  ...
  md_ins_refresh_railcar_btn.click(function (e, p_obj_type) {
    ...
    $('input#md_ins_asset_number').val(
      l_asset_info.instance_id != -1 ? l_asset_info.asset_number : '',
    )
    $('input#md_ins_asset_group').val(
      l_asset_info.instance_id != -1 ? l_asset_info.asset_group : '',
    )
    ...
    l_car_inspections.forEach(function (item) {
      var l_defect = {
        ...
        defect_descr: item.DEFECT_DESCR,
        doc_type: item.DOC_TYPE,
        doc_num: item.DOC_NUM,
        defect_date: item.DEFECT_DATE,
      }
      if (l_prev_inspection_id != item.INSPECTION_ID) {
        l_car_inspections_mas[
          l_car_inspections_mas.length == 0 ? 1 : l_car_inspections_mas.length
        ] = {
          ...
          created_by: item.CREATED_BY,
          created_by_name: item.CREATED_BY_NAME, // add 23.07.2026
          defects: [l_defect],
        }
      } else {
        l_car_inspections_mas[l_car_inspections_mas.length - 1].defects.push(
          l_defect,
        )
      }
      l_prev_inspection_id = item.INSPECTION_ID
    })
    l_car_inspections_mas[0] = {
      inspection_id: '',
      ...
      refusal_to_repair: '',
      created_by: '',
      created_by_name: '', // add 23.07.2026
      defects: [],
      /*,defects:[{defect_id:''
                                             ,defect_code:''
                                             ,defect_descr:''
    ...
    l_car_inspections_mas.forEach(function (item, index) {
      options +=
        ...
        index +
        '">' +
        (item.inspection_id == '' ? '' : item.asset_instance_number) +
        ' ' +
    ...
    if (l_car_conts.length != 0) {
      md_div_ins_cont.append(
        '<label><input style="vertical-align: middle; margin-top: -0.5px;" name="md_ins_cont" checked type="checkbox" val="' +
          l_obj_number +
          '">' +
          l_obj_number +
          '</label>',
  ...
  $('#md_ins_inspection_place').combobox({
    menuMaxHeight: '50em',
    menuWidth: '250',
  })
  $('#md_ins_inspection_person').combobox({
    menuMaxHeight: '50em',
    menuWidth: '250',
  })
  ...
  $('#md_ins_inspection_person').on('select', function () {
    var l_appoint = $('#md_ins_inspection_person > option:selected').attr(
      'data-appoint',
    )
    $('#md_ins_inspection_person_appoint').val(
      l_appoint == 'null' ? '' : l_appoint,
    )
  ...
  $('#md_ins_result').on('select', function () {
    if (
      $(this).children('option:selected').val() == 'Не годен' ||
      $(this).children('option:selected').val() == 'Ограничение'
    ) {
  ...
  $('#tbl_list_of_defects').on(
    'select',
    'tr > td:nth-child(1) > select',
    function () {
      var tr = $(this).parent().parent()
      tr.find('td:nth-child(2)').text(
        $(this).children('option:selected').attr('title'),
      )
      md_disable_btn_save_inspection()
    },
  )
  ...
  $(
    '#md_ins_inspection_place,#md_ins_result,#md_ins_master,#md_ins_inspection_person,#md_ins_status_kurs',
  ).on('select', function () {
    md_disable_btn_save_inspection()
  })
  ...
  md_ins_history.on(
    'change' /*,{p_inspections:l_car_inspections_mas}*/,
    function (e) {
      ...
      $('#md_ins_inspection_place')
        .val(l_inspection.inspection_place)
        .parent()
        .find('input')
        .val(l_inspection.inspection_place)
      $('#md_ins_inspection_person')
        .val(l_inspection.inspection_person)
        .parent()
        .find('input')
        .val(l_inspection.inspection_person)
      $('#md_ins_inspection_person_appoint').val(
        l_inspection.inspection_person_appoint,
      )
      $('#md_ins_master')
        .val(l_inspection.master)
        .parent()
        .find('input')
        .val(l_inspection.master)
      $('#md_ins_priority')
        .val(l_inspection.priority)
        .parent()
        .find('input')
        .val(l_inspection.priority)
      $('#md_ins_result')
        .val(l_inspection.result)
        .parent()
        .find('input')
        .val(l_inspection.result)
      $('#md_ins_status_kurs')
        .val(l_inspection.status_kurs)
        .parent()
        .find('input')
        .val(l_inspection.status_kurs)
      $('#md_ins_comment').val(l_inspection.description)
      $('#md_ins_created_request_status').val(
        l_inspection.created_request_status,
      )
      $('#md_ins_refusal_to_repair').val(l_inspection.refusal_to_repair)
      $('#md_ins_created_by').val(l_inspection.created_by)
      $('#md_ins_inspection_created_users').val(l_inspection.created_by_name) //add 23.07.2026
      ...
      $(
        'input[name="md_ins_cont"][val="' +
          l_inspection.asset_instance_number +
          '"]',
      ).prop('checked', true)
  ...
  if (p_clicked_li !== undefined) {
    ...
    md_ins_refresh_railcar_btn.triggerHandler(
      'click',
      p_clicked_li.attr('data-type'),
    )
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        md_ins_refresh_car_number.autocomplete({
          source: l_cars,
          minLength: 2,
          menuMaxHeight: '50em',
          menuWidth: 80,
    ...
    md_ins_refresh_car_number.keyup(function (e) {
      if (
        check_car_number($(this).val()) ||
        $(this).val() === 'ВЕСЫ-МОД-ЖД-7260SM'
      ) {
        $(this).addClass('true-car-number')
    ...
    if (l_inspections == '') {
      if (l_place === 'ПСП ст. Углеуральская') {
        ...
        if (date_comparison(l_inspections_d_7, l_date, '>')) {
          create_info_modal_dialog_new(
            'Ошибка',
            'Допустимая дата регистрации данных должна быть больше или равна ' +
              l_inspections_d_7,
          )
          return false
        }
    ...
    buttons: {
      Сохранить: {
        ...
        click: function () {
          ...
          var res = md_enter_inspection_ajax(
            md_ins_enter_car_number.val(),
            l_cont,
            l_car_inspections_mas[md_ins_history.val()].inspection_id,
            $('#md_ins_inspection_date').val(),
            $('#md_ins_inspection_place').val(),
            $('#md_ins_inspection_person').val(),
            $('#md_ins_inspection_person_appoint').val(),
            $('#md_ins_master').val(),
            $('#md_ins_priority').val(),
            $('#md_ins_result').val(),
            $('#md_ins_status_kurs').val(),
            $('#md_ins_comment').val(),
            l_defects,
          )
      ...
      Удалить: {
        ...
        click: function () {
          var res = md_delete_inspection_ajax(
            l_car_inspections_mas[md_ins_history.val()].inspection_id,
          )
      ...
      'Отказ от ремонта': {
        ...
        click: function () {
          ...
          md_div.dialog({
            ...
            buttons: {
              Сохранить: function () {
                var res = refusal_to_repair_ajax(
                  l_car_inspections_mas[md_ins_history.val()].inspection_id,
                  $('#md_ins_refusal_to_repair_person').val(),
                )
...
function create_md_fix_device(p_clicked_li) {
  ...
  function get_device_id(p_param) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { params: JSON.stringify(l_param), ajax_action: 'get_device_id' },
      success: function (data) {
        res = data
      },
      error: function (data) {
    ...
    buttons: {
      Сохранить: {
        ...
        click: function () {
          ...
          v_param.inspection_person_appoint = $(
            '#md_ins_inspection_person_appoint',
          ).val()
      ...
      Удалить: {
        ...
        id: 'md_btn_delete_inspection',
        click: function () {
          var res = md_delete_dev_inspection_ajax(
            l_car_inspections_mas[md_ins_history.val()].inspection_id,
          )
      ...
      'Отказ от ремонта': {
        ...
        click: function () {
          ...
          md_div.dialog({
            ...
            buttons: {
              Сохранить: function () {
                var res = refusal_dev_to_repair_ajax(
                  l_car_inspections_mas[md_ins_history.val()].inspection_id,
                  $('#md_ins_refusal_to_repair_person').val(),
                )
...
function md_disable_inspection_add_btns() {
  ...
  if (
    md_ins_history.val() == 0 ||
    md_ins_history.val() == null ||
    md_ins_old_result != 'Не годен'
  ) {
...
function add_defect_tr(
  ...
  function get_document_types() {
    ...
    var l_select = $('<select>').addClass('md-select').css({ width: '14em' })
    l_select.append($('<option>'))
    $.each(l_places, function (i, item) {
      l_select.append(
        $('<option>').val(item.CODE).prop('title', item.DESCR).text(item.CODE),
      )
    })
    return l_select
  }
  ...
  if (
    ($('input#md_ins_enter_car_number').val() === 'ВЕСЫ-МОД-ЖД-7260SM' &&
      $('#tbl_list_of_defects').find('tr').length === 0) ||
    $('input#md_ins_enter_car_number').val() !== 'ВЕСЫ-МОД-ЖД-7260SM'
  ) {
    var tr = $('<tr>')
    ...
    tr.append(
      $('<td>')
        .text(p_defect_id == null ? '' : p_defect_id)
        .hide(),
    )
    tr.append(
      '<td><div class="deleteImage deleteImage13px" title="Удалить" onclick="$(this).parent().parent().remove(); md_disable_btn_save_inspection();"></div></td>',
    )
    ...
    create_info_modal_dialog_new(
      'Предупреждение',
      'По активу "ВЕСЫ-МОД-ЖД-7260SM" можно добавлять только ОДНУ неисправность для осмотра!',
    )
  }
}

function add_defect_dev_tr(
  p_defect_id,
  p_defect_code,
  p_defect_descr,
  p_doc_type,
...
function md_disable_btn_save_inspection() {
  ...
  if (
    $('#md_ins_inspection_date').hasClass('red_bckg_color') ||
    $('#md_ins_inspection_date').val() == '' ||
    $('#md_ins_inspection_place').val() == '' ||
    l_result == '' ||
    $('#md_ins_master').val() == '' ||
    ...
    ((l_result == 'Не годен' || l_result == 'Ограничение') &&
      (!l_flag_fill_defects || l_defects_count == 0)) ||
    ($('input[name="md_ins_cont"]').length > 0 &&
      $('input[name="md_ins_cont"]:checked').length == 0) ||
    $('#md_ins_old_result').val() == 'Не годен' ||
    ($('#md_ins_enter_car_number').val() == '' &&
      $('input[name="md_ins_cont"]').length != 1) ||
    (user_id != $('#md_ins_created_by').val() &&
      r_enter_inspection_add == false &&
      $('#md_ins_created_by').val() != '')
  ) {
    $('#md_btn_save_inspection').prop('disabled', true)
  } else {
...
function get_select_places(p_id) {
  ...
  var result =
    '<select id="' + p_id + '" class="required md-select" style="width: 14em;">'
  result += '<option value=""></option>'
  $.each(l_places, function (i, item) {
    result +=
      '<option value="' +
...
function get_inspection_persons(p_id) {
  ...
  var result =
    '<select id="' + p_id + '" class="md-select required" style="width: 16em;">'
  result += '<option value=""></option>'
...
function get_asset_info(p_car_number) {
  ...
  $.ajax({
    ...
    dataType: 'text',
    async: false,
    data: { car_number: p_car_number, ajax_action: 'get_asset_info' },
    success: function (data) {
  ...
  if (l_asset_info.length != 1) {
    ...
    l_asset_info_obj = {
      instance_id: l_asset_info[0].INSTANCE_ID,
...
function get_device_info(p_car_number) {
  ...
  if (l_asset_info.length != 1) {
    ...
    l_asset_info_obj = {
      instance_id: l_asset_info[0].INSTANCE_ID, // id актива
...
function get_priority(p_id) {
  ...
  var result =
    '<select id="' + p_id + '" class="md-select" style="width: 10em;">'
  result += '<option value=""></option>'
  $.each(l_places, function (i, item) {
...
function get_ins_results(p_id, p_few_cars_flag) {
  var result =
    '<select id="' + p_id + '" class="md-select required" style="width: 8em;">'
  result += '<option value=""></option>'
  $.each(g_inspection_results, function (i, item) {
    var disabled = ''
    if (
      p_few_cars_flag == 'Y' &&
      (item.CODE == 'Не годен' || item.CODE == 'Ограничение')
    ) {
      disabled = 'disabled'
    }
    result +=
...
function get_status_kurs(p_id) {
  ...
  var result =
    '<select id="' + p_id + '" class="md-select" style="width: 14em;">'
  result += '<option value=""></option>'
  $.each(l_places, function (i, item) {
    result +=
      '<option value="' +
...
function create_md_enter_inspection_for_few_cars(p_obj_type) {
  ...
  function get_tr_for_inspection_table(
    p_obj_type,
    p_number,
    p_obj_number,
    p_freight,
  ...
  function add_car_in_inspection_table(p_type_obj) {
    ...
    if (obj_number.length < 8 && p_type_obj == 'railcar') {
      ...
      ins_tbl.find('tr td:contains("' + obj_number + '")').length !== 0
    ) {
      car_number_input.addClass('red_bckg_color')
      alert(
        (p_type_obj == 'railcar' ? 'Вагон ' : 'Контейнер ') +
          obj_number +
          ' уже добавлен!',
      )
    } else {
      $.ajax({
        ...
        async: false,
        data: {
          car_number: obj_number,
          ajax_action:
            p_type_obj == 'railcar'
              ? 'get_add_info_for_car'
              : 'get_add_info_for_cont',
        },
        success: function (data) {
          var records = JSON.parse(data)
  ...
  /* Сохраняем данные */
  function md_enter_inspection_for_few_cars_ajax(
    p_obj_type,
    p_obj_numbers,
    p_cars_nsi,
    p_inspection_date,
  ...
  var l_md_div = $('<div/>')
    ...
    .attr(
      'title',
      p_obj_type == 'railcar' ? 'Осмотр вагонов' : 'Осмотр контейнеров',
    )
    .appendTo('body') // Присоединяем наше меню к body документа:
    .append(
      '<div style="display: table;">' +
  ...
  $('#md_ins_inspection_person').on('select', function () {
    $('#md_ins_inspection_person_appoint').val(
      $('#md_ins_inspection_person > option:selected').attr('data-appoint'),
    )
    //alert($('#md_ins_inspection_person > option:selected').val());
  })
...
function create_md_return_from_PSP() {
  function md_return_from_PSP() {
    ...
    this.show_window = function () {
      ...
      md_div.error_correction_blok = $('<div>', {
        css: { display: 'table' },
      }).appendTo(md_div)
      md_div.return_correction_reason = $('<select>')
        .css({ width: '250px' })
        .append('<option value=""></option>')
      md_div.station_operator = $('<input>', {
        type: 'text',
        class: 'text ui-widget-content ui-corner-all',
        css: { width: '200px' },
      })
      ...
      md_div.error_correction_blok.append(
        $('<div>', { class: 'border', css: { clear: 'both' } })
          .append(
            $('<div>', { class: 'header', css: { width: '72px' } }).text(
              'Возврат',
            ),
          )
          .append(
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          result.forEach(function (item) {
            md_div.return_correction_reason.append(
      ...
      railcar_table.spec_check_car_number = function (p_car_number) {
        var find_result = $('#comingCarsTree').find(
          'li[data-id=' + p_car_number + '][data-from_station_id=2]',
        )
      ...
      md_div.disable_ok_btn = function () {
        if (
          md_div.return_time.hasClass('red_bckg_color') ||
          md_div.return_time.val() == '' ||
          md_div.return_reason.val() == '' ||
          railcar_table.cars_count == 0
        ) {
          $('#md_ok_btn').prop('disabled', true)
        } else {
      ...
      md_div.check_cars_train = function () {
        $.ajax({
          ...
          async: false,
          data: {
            cars_number: railcar_table.get_cars_in_table(),
          ...
          success: function (result) {
            if (result != '') {
              ...
              result == '' &&
              md_div.return_reason.val() == 'mistake'
            ) {
      ...
      md_div.return_time.blur(function () {
        ...
        if (
          ...
          var pattern = /(\d{2})\.(\d{2})\.(\d{4}) (\d{2})\:(\d{2})/
          var l_return_time_d = new Date(
            l_return_time.replace(pattern, '$3-$2-$1T$4:$5:00'),
          )
          var l_return_time_last_d = new Date(
            l_return_time_last.replace(pattern, '$3-$2-$1T$4:$5:00'),
          )
        ...
        buttons: {
          Вернуть: {
            ...
            click: function () {
              ...
              if (
                return_from_PSP_ajax(
                  cars_in_table,
                  md_div.return_time.val(),
                  md_div.return_reason.val(),
                  md_div.return_comment.val(),
                  md_div.return_correction_reason.val(),
                  md_div.station_operator.val(),
                ) === 'done'
              ) {
...
function create_md_weigh_import() {
  function railcar_table_for_weigh() {
    ...
    return_table.cars_table = $('<table>', {
      class: 'weigh_import_table cars_table',
    }).append($('<tbody>'))
    return_table.append(
      $('<div>', {
        class: 'modalDialogContainer',
    ...
    this.add_cars_in_table = function (
      ...
      tr.comment_input = $('<input>')
        ...
        .addClass('text ui-widget-content ui-corner-all')
      tr.check_input = $('<input>').attr({
        type: 'checkbox',
        checked: 'checked',
      })

      // add 14.01.2021 Checkbox для акта взвешивания
      tr.for_akt = $('<input>').attr({ type: 'checkbox' })
    ...
    this.get_cars_in_table = function () {
      ...
      tr_mas.forEach(function (item) {
        if (item.car_number_input.hasClass('true-car-number')) {
          var l_com =
            item.comment_input.val() == '' ? '_' : item.comment_input.val()
          var l_check = item.check_input.prop('checked') == true ? '1' : '0'
          var l_car_number =
            item.car_number_input.val() == ''
              ? '_'
              : item.car_number_input.val()
          var l_weight =
            item.weight_input.val() == '' ? '_' : item.weight_input.val()
          var l_date = item.date_input.val() == '' ? '_' : item.date_input.val()
          var l_speed =
            item.speed_input.val() == '' ? '_' : item.speed_input.val()
          var l_weight_type =
            item.weight_type.val() == '' ? '_' : item.weight_type.val()
    ...
    this.get_flag_exist_empty_fields = function (p_manual_input) {
      ...
      tr_mas.forEach(function (item) {
        if (
          item.weight_input.val() == '' ||
          (p_manual_input &&
            (!item.car_number_input.hasClass('true-car-number') ||
              item.speed_input.val() == '' ||
              item.date_input.val() == '' ||
              item.date_input.hasClass('red_bckg_color') ||
              item.weight_input.hasClass('red_bckg_color')))
  ...
  var md_div = $('<div/>').attr('title', 'Импорт взвешиваний').appendTo('body') // Присоединяем наше меню к body документа:

  md_div.general_comment = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
    css: { width: '300px' },
  })
  md_div.train = $('<select>', {
    css: { width: '170px', 'text-align': 'right' },
  })
  md_div.manual_input = $('<input>', { type: 'checkbox' })
  md_div.train_date = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
    css: { width: '9em' },
  }).attr('maxlength', '16')
  md_div.train_speed = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
    css: { width: '50px' },
  }).attr('maxlength', '6')
  md_div.add_car_btn = $('<input>', { type: 'button', class: 'btnFind' }).val(
    'Добавить вагон',
  )
  ...
  md_div.add_car_div = $('<div>', { css: { 'text-align': 'left' } }).append(
    md_div.add_car_btn,
  )
  ...
  md_div.append(
    $('<div>', { css: { display: 'table' } }).append(
      $('<div>', { class: 'attr', css: { border: 'none', width: '484px' } })
        ...
        .append(
          $('<div>').append($('<label>').text('Поезд')).append(md_div.train),
        )
        .append(
        ...
        .append(md_div.train_date_div)
        .append(md_div.train_speed_div)
        .append(md_div.add_car_div),
    ),
  )
  ...
  l_cars_from_scale.forEach(function (item) {
    ...
    if (l_prev_id != item.TRAIN_ID) {
      l_cars_from_scale_mas[item.TRAIN_ID] = {
        train_date: item.TRAIN_DATE,
        cars: [l_car],
      }
    } else {
      if (l_cars_from_scale_mas[item.TRAIN_ID] != undefined) {
        // add 30.09.2022
  ...
  l_cars_from_scale_mas.forEach(function (item, item_id) {
    md_div.train.append(
      '<option value="' + item_id + '">' + item.train_date + '</option>',
    )
  })
  ...
  md_div.add_car_btn.click(function () {
    railcar_table.add_cars_in_table(
      ...
      null,
      null,
      md_div.train_date.val(),
      'Динамика',
      md_div.train_speed.val(),
      true,
    )
  })
      ...
      Загрузить: {
        ...
        click: function () {
          ...
          if (l_check_period == '0') {
            create_info_modal_dialog_new(
              'Оповещение',
              'Для даты взвешивания нет открытого периода! Загрузить взвешивания не возможно!',
            )
          } else {
            if (
              railcar_table.get_flag_exist_empty_fields(
                md_div.manual_input.prop('checked'),
              )
            ) {
              var l_train_id = md_div.train.val()
              if (md_div.manual_input.prop('checked')) {
                l_train_id = null
              }
              ...
              var l_ajax_result = load_weigh_import_ajax(
                l_train_id,
                l_general_com,
                l_cars,
              )
              ...
              create_info_modal_dialog_new(
                'Ошибка',
                'Заполните все обязательные поля в таблице!',
              )
            }
          }
          stop_loading_animation()
...
function create_md_weight_result(p_clicked_li) {
  function railcar_table_for_weigh() {
    ...
    return_table.cars_table = $('<table>', {
      class: 'weigh_import_table_add cars_table',
    }).append($('<tbody>'))
    return_table.append(
      $('<div>', {
        class: 'modalDialogContainer',
    ...
    this.add_cars_in_table = function (
      ...
      l_weight_car.weight_date = $('<td>')
        .appendTo(l_weight_car)
        .text(p_weight_date)
      l_weight_car.weight_type = $('<td>')
        .appendTo(l_weight_car)
        .text(p_weight_type)
      l_weight_car.weight_speed = $('<td>')
        .appendTo(l_weight_car)
        .text(p_weight_speed)
      l_weight_car.weight = $('<td>').appendTo(l_weight_car).text(p_weight)
      l_weight_car.weight_cor = $('<td>')
        .appendTo(l_weight_car)
        .text(p_weight_cor)
      ...
      if (r_weighing_dispatcher) {
        ...
        l_weight_car.weight_date.click(function () {
          change_weight_car(l_weight_car)
        })
      }
    ...
    function change_weight_car(p_weight_car) {
      ...
      md_rep_div.weight_date = $('<input>', {
        type: 'text',
        class: 'text ui-widget-content ui-corner-all',
        css: { width: '12em' },
      }).val(l_weight_car.p_weight_date)
      ...
      md_rep_div.weight_type = $('<input>', {
        type: 'text',
        class: 'text ui-widget-content ui-corner-all',
        css: { width: '12em' },
      }).val(l_weight_car.p_weight_type)
      md_rep_div.weight_type.prop('disabled', true)

      md_rep_div.weight_speed = $('<input>', {
        type: 'text',
        class: 'text ui-widget-content ui-corner-all',
        css: { width: '12em' },
      }).val(l_weight_car.p_weight_speed)
      md_rep_div.weight_speed.prop('disabled', true)

      md_rep_div.weight = $('<input>', {
        type: 'text',
        class: 'text ui-widget-content ui-corner-all',
        css: { width: '12em' },
      }).val(l_weight_car.p_weight)
      md_rep_div.weight.prop('disabled', true)

      md_rep_div.weight_cor = $('<input>', {
        type: 'text',
        class: 'text ui-widget-content ui-corner-all',
        css: { width: '12em' },
      }).val(l_weight_car.p_weight_cor)
      ...
      md_rep_div.comment = $('<input>', {
        type: 'text',
        class: 'text ui-widget-content ui-corner-all',
        css: { width: '12em' },
      }).val(l_weight_car.p_comment)
      ...
      var l_div_weight_date = $('<div>')
        ...
        .append(
          $('<label>', {
            text: 'Дата взвешивания ',
            class:
              'car_scale-window-attr-item-text car_scale-window-attr-item-text-left',
          }),
        )
        ...
        buttons: {
          Сохранить: function () {
            ...
            if (f_res_mas[0] == 'done') {
              ...
              create_info_modal_dialog_new(
                'Ошибка',
                'Процедура завершилась с ошибкой! Причина:' + f_res_mas[1],
              )
              $(this).dialog('close')
            }
          },
    ...
    this.get_cars_in_table = function () {
      ...
      tr_mas.forEach(function (item) {
        ...
        var l_check = item.check_input.prop('checked') == true ? '1' : '0'
        var l_car_number =
          item.car_number_input.val() == '' ? '_' : item.car_number_input.val()
        var l_weight =
          item.weight_input.val() == '' ? '_' : item.weight_input.val()
  ...
  function add_report_window() {
    function run_report_disl_007(p_from_date, p_to_date, p_owner) {
      var win = window.open(
        'xx_etw_disl_007/xx_etw_disl_007.php?' +
          'p_from_date=' +
          p_from_date +
          '&' +
          'p_to_date=' +
    ...
    md_rep_div.start_date_from = $('<input>', {
      type: 'text',
      requir: 'Y',
      class: 'text ui-widget-content ui-corner-all required',
    })
    ...
    md_rep_div.start_date_to = $('<input>', {
      type: 'text',
      requir: 'Y',
      class: 'text ui-widget-content ui-corner-all required',
    })
  ...
  var l_car_input = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
    css: { width: '5em' },
  })
  var l_refresh_btn = $('<button>', {
    class: 'button',
    css: { 'margin-left': '1em' },
  }).append(
    $('<span>', { class: 'button-text button-text-size-2' }).text('Обновить'),
  )
  var l_refresh_railcar_div = $('<div>', {
    class: 'border',
    css: { display: 'table' },
  }).append(
    $('<div>', { css: { float: 'left' } })
      ...
      success: function (data) {
        ...
        l_cars_from_scale.forEach(function (item) {
          railcar_table.add_cars_in_table(
            ...
            item.TRAIN_ID,
            item.DATE_W,
            /*'Динамика'*/ item.WEIGHING_TYPE,
            item.SPEED,
            item.WEIGHT,
            item.WEIGHT_COR,
            item.DESCR,
            item.PREV_MAX_DATE_W,
            item.FOR_AKT,
            item.PK_ID,
            item.SCALE_TYPE,
          )
...
function create_md_move_cont(p_clicked_li) {
  $('.context-menu').remove()
  function create_md_move_cont_ajax(
    p_conts,
    p_place_type,
    p_place_id,
    p_oper_date,
        ...
        $('<div>', { class: 'border', css: { clear: 'both' } })
          ...
          .append(
            $('<div>', { css: { display: 'inline-table' } }).append(
              $('<div>', {
                class: 'attr',
                css: { border: 'none', width: '470px', 'text-align': 'left' },
              }).append(
                $('<div>')
                  .append(
                    $('<div>', { css: { 'text-align': 'left' } }).append(
                      md_div.new_place_type,
                    ),
                  )
                  .append(
                    $('<div>', {
                      css: { 'text-align': 'left', 'margin-top': '5px' },
                    }).append(md_div.new_place_station),
                  )
                  .append(
                    $('<div>', {
                      css: { 'text-align': 'left', 'margin-top': '5px' },
                    }).append(md_div.new_place),
                  ),
              ),
            ),
          ),
  ...
  md_div.new_place_type.select(function () {
    fill_select_with_places(
      md_div.new_place,
      md_div.new_place_station.val(),
      md_div.new_place_type.val(),
    )
    ...
    buttons: {
      Принять: {
        ...
        click: function () {
          ...
          var l_res = create_md_move_cont_ajax(
            l_conts_in_table_str,
            md_div.new_place_type.val(),
            md_div.new_place.val(),
            md_div.operation_date.val(),
            md_div.comment.val(),
            ...
            if (md_div.new_place_type.val() === 'area') {
              if (
                ...
                if (
                  ...
                  l_conts_in_table.forEach(function (cont_number) {
                    $('li[data-id="' + cont_number + '"][data-type="cont"]')
                      .detach()
                      .appendTo(newParentCont)
                  })
                } else {
...
function create_md_export_samples() {
  ...
  function fill_product_for_freight(p_select, p_freight) {
    ...
    $.ajax({
      ...
      async: false,
      data: {
        add_data: JSON.stringify(v_params),
  ...
  function table_for_weigh_from_shop_samples() {
    ...
    return_table.cars_table = $('<table>', {
      class: 'table_for_weigh_from_shop_samples cars_table',
    }).append($('<tbody>'))
    function get_sample_tbl_header(p_params) {
      ...
      $.ajax({
        ...
        async: false,
        data: {
          add_data: JSON.stringify(p_params),
    ...
    this.load_table = function (p_product, p_item_code_oebs) {
      function getObjForHeader(p_array, p_value, p_car_overload) {
        ...
        $.each(p_array, function (i, item) {
          ...
          objHeader = $('<input>', {
            type: 'input',
            css: { width: '80px' },
            class: item.REQUIRED_T === 'Y' ? 'required' : '',
          })
          if (p_car_overload === 'Y') {
          ...
          if (dataType === 'NUMBER') {
            ...
            objHeader.attr(
              'placeholder',
              item.MIN_VALUE_NUM + ' - ' + item.MAX_VALUE_NUM,
            )
            objHeader.on('input', function () {
              if (this.value !== '' && this.value !== null) {
                ...
                if (
                  !isNumber(value) ||
                  !(
                    value >= parseInt(item.MIN_VALUE_NUM) &&
                    value <= parseInt(item.MAX_VALUE_NUM)
                  )
                ) {
                  this.value = ''
                  create_info_modal_dialog_new(
                    'Ошибка',
                    'Недопустимое значение! Разрешен ввод чисел в диапазоне от ' +
                      item.MIN_VALUE_NUM +
                      ' до ' +
                      item.MAX_VALUE_NUM,
                  )
            ...
            if (Object.keys(samplesMS).length > 0) {
              ...
              objHeader = $('<select>', {
                class: item.REQUIRED_T === 'Y' ? 'required' : 'nop',
              })
      ...
      $.each(dataInfo, function (i, item) {
        ...
        table_samples.tr.append(
          $('<td>').text(item.CONT_NUMBER === null ? '' : item.CONT_NUMBER),
        )
        table_samples.tr.append(
          $('<td>').text(item.LOT_NUMBER === null ? '' : item.LOT_NUMBER),
        )
        table_samples.tr.append(
          $('<td>').text(item.SAMPLE_NO === null ? '' : item.SAMPLE_NO),
        )
        ...
        $.each(samplesMS, function (j, jItem) {
          ...
          var arrayForHeader = []
          arrayForHeader.push(
            table_h.find((itemH) => itemH.TEST_ID === sampleData.OGPTestID),
          )

          sampleData.headerArray = arrayForHeader
          sampleData.OGPInput = getObjForHeader(
            arrayForHeader,
            jItem.value,
            jItem.car_overload,
          )
        ...
        if (item.checkbox.prop('checked') === true) {
            ...
            if (samOGP.headerArray[0].DATA_TYPE === 'NUMBER') {
              if (OGPValues !== '' && OGPValues !== null) {
                if (
                  ...
                  !(
                    OGPValues >=
                      parseInt(samOGP.headerArray[0].MIN_VALUE_NUM) &&
                    OGPValues <= parseInt(samOGP.headerArray[0].MAX_VALUE_NUM)
                  )
                ) {
                  l_result.done = false
          ...
          if (sOGPErrorVal !== '') {
            l_result.mistakes.push(
              'Для строки ' +
                item.NUM +
                ' введено неверное значение для полей: (' +
                sOGPErrorVal +
                ')',
            )
          }
          if (sOGPSelect !== '') {
            l_result.mistakes.push(
              'Для строки ' +
                item.NUM +
                ' выбрано значение,которое отсутствует в справочнике. Дополнить справоник для (' +
                sOGPSelect +
                ')',
            )
          }
          if (sControlWeight !== '') {
            l_result.mistakes.push(
              'Для строки ' + item.NUM + ' ' + sControlWeight,
            )
          }
        }
      })
  ...
  function export_samples_info(p_add_data) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { add_data: p_add_data, ajax_action: 'export_samples_info' },
      success: function (data) {
        res = data
      },
      error: function (data) {
  ...
  md_div.freight = $('<select>', {
    class: 'required',
    css: { width: '170px', 'text-align': 'right' },
  })
  md_div.product = $('<select>', {
    class: 'required',
    css: { width: '170px', 'text-align': 'right' },
  })
  md_div.date_from = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all required',
    css: { width: '130px' },
  }).val(l_date_from_str)
  md_div.date_to = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all required',
    css: { 'margin-left': '5px', width: '130px' },
  }).val(l_date_to_str)
  md_div.status = $('<select>', {
    css: { width: '170px', 'text-align': 'right' },
  })
  md_div.type = $('<select>', {
    css: { width: '170px', 'text-align': 'right' },
  })
  md_div.batch_num = $('<input>', {
    css: { width: '100px', 'text-align': 'right' },
  })
  md_div.refresh_btn = $('<input>', { type: 'button', class: 'btnFind' }).val(
    'Обновить',
  )

  md_div.refresh_div = $('<div>', { css: { 'text-align': 'left' } }).append(
    md_div.refresh_btn,
  )
  ...
  md_div.freight.append(
    '<option value="' + 'Карбамид' + '">' + 'Карбамид' + '</option>',
  )
  md_div.freight.append(
    '<option value="' + 'Метанол' + '">' + 'Метанол' + '</option>',
  )
  ...
  md_div.append(
    $('<div>', { css: { display: 'table' } }).append(
      $('<div>', { class: 'attr', css: { border: 'none', width: '470px' } })
        .append(
          $('<div>').append($('<label>').text('Груз')).append(md_div.freight),
        )
        .append(
          $('<div>')
        ...
        click: function () {
          ...
          if (l_result.done == false) {
            ...
            if (result[0] == 'done') {
              create_info_modal_dialog_new(
                'Оповещение',
                'Процедура завершилась успешно!' + '<br>' + result[1],
              )
              $('#md_export_samples_btn').prop('disabled', false)
              $(this).dialog('close')
            } else {
              create_info_modal_dialog_new(
                'Предупреждение',
                'Процедура завершилась с ошибкой!!!' + '<br>' + result[1],
              )
              $('#md_export_samples_btn').prop('disabled', false)
            }
            //console.log(l_oebs_json);
...
function create_md_export_shop_info() {
  function run_report() {
    ...
    var win = window.open(
      'xx_etw_disl_009/xx_etw_disl_009.php?' +
        'freight=' +
        l_freight +
        '&' +
        'date_from=' +
  ...
  function get_select_freight_oebs(p_freight, p_product, p_cont_number) {
    ...
    if (p_freight.toUpperCase() === 'КФК') {
      result_select.append(
        '<option selected value="' + 'ГП0043' + '">' + 'ГП0043' + '</option>',
      )
      ...
      result_select.append(
        '<option value="' + 'ГП0511' + '">' + 'ГП0511' + '</option>',
      )
      result_select.append(
        '<option value="' + 'ГП0515' + '">' + 'ГП0515' + '</option>',
      ) // add 20.06.2020
      result_select.append(
        '<option value="' + 'ГП0522' + '">' + 'ГП0522' + '</option>',
      ) // add 20.06.2020
    } else if (p_freight.toUpperCase() === 'КАРБАМИД') {
      result_select.append(
        '<option value="' + 'ГП0098' + '">' + 'ГП0098' + '</option>',
      ) // add 13.12.2022
    } else if (p_freight.toUpperCase() === 'МЕТАНОЛ') {
      result_select.append(
        '<option value="' + 'ГП0011' + '">' + 'ГП0011' + '</option>',
      ) // add 19.03.2024 по наряду 0000048724
  ...
  function export_shop_info_ajax(p_shop_info) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { shop_info: p_shop_info, ajax_action: 'export_shop_info' },
      success: function (data) {
        res = data
      },
      error: function (data) {
  ...
  function table_for_weigh_from_shop_info() {
    ...
    return_table.cars_table = $('<table>', {
      class: 'table_for_weigh_from_shop_info cars_table',
    }).append($('<tbody>'))
    return_table.append(
      $('<div>', {
        class: 'modalDialogContainer',
    ...
    this.add_cars_in_table = function (p_cars_from_shop_mas) {
      p_cars_from_shop_mas.forEach(function (item, index) {
        ...
        tr.freight_name_oebs = get_select_freight_oebs(
          item.freight_name,
          item.product,
          item.cars[0].cont_number,
        )
        ...
        tr.who_zayavka
          .css({ width: '10em' })
          .prop('disabled', true)
          .val(item.who_zayavka)
        if (item.who_zayavka === null && item.who_zayavka_add !== null) {
          tr.who_zayavka.append(
            '<option selected value="' +
              item.who_zayavka_add +
              '">' +
              item.who_zayavka_add +
              '</option>',
          )
        ...
        tr.append(
          $('<td>').attr('rowspan', item.cars.length).append(tr.checkbox),
        )
        tr.append($('<td>').attr('rowspan', item.cars.length).append(index))
        tr.append(
          $('<td>').attr('rowspan', item.cars.length).append(tr.freight_name),
        )
        tr.append(
          $('<td>')
            .attr('rowspan', item.cars.length)
            .append(tr.freight_name_oebs),
        )
        tr.append(
          $('<td>').attr('rowspan', item.cars.length).append(tr.batch_no),
        )
        tr.append(
          $('<td>').attr('rowspan', item.cars.length).append(tr.date_post),
        )
        tr.append(
          $('<td>').attr('rowspan', item.cars.length).append(tr.date_start),
        )
        tr.append(
          $('<td>').attr('rowspan', item.cars.length).append(tr.date_end),
        )
        tr.append(
          $('<td>')
            .attr('rowspan', item.cars.length)
            .append(tr.date_zayavka_uvod),
        )
        tr.append(
          $('<td>').attr('rowspan', item.cars.length).append(tr.date_uvod),
        )
        ...
        l_car.weight_net = $('<input>', {
          ...
          .prop('disabled', true)
          .val(item.cars[0].weight_net.replace(',', '.'))
        l_car.refusal = $('<input>', {
          class: 'text ui-widget-content ui-corner-all',
          type: 'text',
          maxlength: '200',
          ...
          .prop('disabled', true)
          .val(item.cars[0].refusal)

        tr.shops.push(l_car)

        tr.append(
          $('<td>', { css: { width: '80px', 'font-weight': '700' } }).append(
            l_car.car_number,
          ),
        )
        tr.append(
          $('<td>', { css: { width: '100px', 'font-weight': '700' } }).append(
            l_car.cont_number,
          ),
        )
        tr.append(
          $('<td>', { css: { width: '55px' } }).append(l_car.weight_net),
        )
        tr.append($('<td>', { css: { width: '120px' } }).append(l_car.refusal))

        tr.append(
          $('<td>').attr('rowspan', item.cars.length).append(tr.who_looked),
        )
        tr.append(
          $('<td>').attr('rowspan', item.cars.length).append(tr.who_start),
        )
        tr.append(
          $('<td>').attr('rowspan', item.cars.length).append(tr.who_end),
        )
        tr.append(
          $('<td>').attr('rowspan', item.cars.length).append(tr.who_zayavka),
        )
        ...
        item.cars.forEach(function (item, index) {
          if (index != 0) {
            ...
            tr_child.append(
              $('<td>', {
                css: { width: '80px', 'font-weight': '700' },
              }).append(l_car.car_number),
            )
            tr_child.append(
              $('<td>', {
                css: { width: '100px', 'font-weight': '700' },
              }).append(l_car.cont_number),
            )
            tr_child.append(
              $('<td>', { css: { width: '55px' } }).append(l_car.weight_net),
            )
            tr_child.append(
              $('<td>', { css: { width: '120px' } }).append(l_car.refusal),
            )
        ...
        if (item.checkbox.prop('checked') === true) {
          item.shops.forEach(function (shop) {
            if (shop.weight_net.val() !== '0') {
              l_result +=
                shop.shop_info_id + ':' + item.freight_name_oebs.val() + '|'
            }
          })
        }
      })
      ...
      tr_mas.forEach(function (item) {
        if (item.checkbox.prop('checked') === true) {
          if (item.batch_no != null) {
            l_result.done = false
            l_result.mistakes.push(
              'Погрузка с номером ' + item.num_index + ' уже отражена в OEBS!',
            )
            //alert('Погрузка с номером '+item.num_index+' уже отражена в OEBS!');
          }
          if (
            item.freight_name_oebs.val() == '' ||
            item.freight_name_oebs.val() == '0'
          ) {
            l_result.done = false
            l_result.mistakes.push(
              'В погрузке с номером ' +
                item.num_index +
                ' не указан груз для OEBS!',
            )
          ...
          if (
            !(
              item.freight_name.val() == 'КФК' ||
              item.freight_name.val() == 'Формалин' ||
              item.freight_name.val() == 'Карбамид' ||
              item.freight_name.val() == 'Метанол'
            )
          ) {
            l_result.done = false
            l_result.mistakes.push(
              'В погрузке с номером ' +
                item.num_index +
                ' указан груз для которого отражение в OEBS не предусмотрено!',
            )
            ...
            if (!(item.freight_name.val() == 'Карбамид')) {
              if (child_item.cont_number == null) {
                ...
                l_result.mistakes.push(
                  'В погрузке с номером ' +
                    item.num_index +
                    ' не указан номер контейнера!',
                )
                //alert('В погрузке с номером '+item.num_index+' не указан номер контейнера!');
              }
              if (
                child_item.cont_number != null &&
                child_item.car_number == null
              ) {
                l_result.done = false
                l_result.mistakes.push(
                  'В погрузке с номером ' +
                    item.num_index +
                    ' не указана платформа для контейнера!',
                )
                //alert('В погрузке с номером '+item.num_index+' не указан номер контейнера!');
              }
              if (
                child_item.cont_number != null &&
                child_item.cont_count == 1
              ) {
                l_result.done = false
                l_result.mistakes.push(
                  'В погрузке с номером ' +
                    item.num_index +
                    ' на платформе находится только один контейнер!',
                )
                //alert('В погрузке с номером '+item.num_index+' не указан номер контейнера!');
              }
            }
            if (
              child_item.weight_net.val() == '0' &&
              (child_item.refusal.val() == '' ||
                child_item.refusal.val() == null)
            ) {
              l_result.done = false
              l_result.mistakes.push(
                'В погрузке с номером ' +
                  item.num_index +
                  ' указан нулевой вес груза!',
              )
  ...
  var md_div = $('<div/>').attr('title', 'Экспорт в OEBS').appendTo('body')
  ...
  md_div.refresh_btn.click(function () {
    if (
      md_div.date_from.hasClass('red_bckg_color') ||
      md_div.date_from.val() == '' ||
      md_div.date_to.hasClass('red_bckg_color') ||
      md_div.date_to.val() == ''
    ) {
      create_info_modal_dialog_new(
        'Предупреждение',
      ...
      l_cars_from_shop.forEach(function (item) {
        ...
        if (l_prev_id != item.ID) {
          l_cars_from_shop_mas[item.ID] = {
            ...
            who_end: item.WHO_END,
            who_zayavka: item.WHO_ZAYAVKA,
            who_looked_add: item.WHO_LOOKED_ADD,
            who_start_add: item.WHO_START_ADD,
            who_end_add: item.WHO_END_ADD,
            who_zayavka_add: item.WHO_ZAYAVKA_ADD,
      ...
      'Экспортировать в OEBS': {
        ...
        click: function () {
          ...
          if (l_result.done == false) {
            ...
            //return;
            var l_result = export_shop_info_ajax(
              export_table.get_check_elements(),
            )
            if (l_result.substr(0, 4) == 'done') {
              ...
              create_info_modal_dialog_new(
                'Предупреждение',
                'Процедура завершилась с ошибкой!!!' + '<br>' + l_result,
              )
            }
          }
...
function create_return_invoice(p_clicked_li) {
  ...
  function table_for_cont_csl() {
    ...
    return_table.cars_table = $('<table>', {
      class: 'table_for_cont_csl cars_table',
    }).append($('<tbody>'))
    return_table.append(
      $('<div>', {
        class: 'modalDialogContainer',
    ...
    this.add_row_in_table = function (
      p_seal_type,
      p_seal_marks,
      p_seal_quantity,
      p_seal_year,
      ...
      tr.seal_owner_type.val(
        p_seal_owner_type === undefined ? '2' : p_seal_owner_type,
      )
      ...
      tr.append(
        $('<td>')
          .append(tr.seal_marks_part1)
          .append(tr.seal_marks_part2)
          .append(tr.seal_marks_part3),
      )
    ...
    this.get_rows = function () {
      ...
      return_table.conts.children().each(function () {
        ...
        conts_obj[cont_number].forEach(function (item) {
          ...
          var seal_marks =
            item.seal_marks_part1.val() +
            item.seal_marks_part2.val() +
            item.seal_marks_part3.val()
  ...
  function get_claim_ajax(p_car_number) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { car_number: p_car_number, ajax_action: 'get_claim' },
      success: function (data) {
        res = data
      },
      error: function (data) {
  ...
  function fill_select_with_graph(p_select, p_claim_number) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { claim_number: p_claim_number, ajax_action: 'get_graph_pod' },
      success: function (data) {
        var records = JSON.parse(data)
        $.each(records, function (i, item) {
          var l_option = $('<option>')
  ...
  function fill_select_with_otpr(p_select, p_claim_number) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { claim_number: p_claim_number, ajax_action: 'get_claim_otpr' },
      success: function (data) {
        var records = JSON.parse(data)
        $.each(records, function (i, item) {
          var l_option = $('<option>')
            .text(item.DESCR + ' своб. ваг = ' + item.FREE_CAR_COUNT)
            .val(item.OTPR_NOM)
            .attr('telefon', item.TELEFON)
          if (records.length == 1) {
            l_option.attr('selected', true)
          }
          p_select.append(l_option)
  ...
  function fill_select_with_recip_address(p_select, p_claim_number) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { claim_number: p_claim_number, ajax_action: 'get_recip_address' },
      success: function (data) {
        var records = JSON.parse(data)
        $.each(records, function (i, item) {
  ...
  function get_front_end_id_ajax(p_car_number) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { car_number: p_car_number, ajax_action: 'get_front_end_id' },
      success: function (data) {
        res = data
      },
      error: function (data) {
  ...
  function get_send_inv_number_ajax(p_car_number) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { car_number: p_car_number, ajax_action: 'get_send_inv_number' },
      success: function (data) {
        res = data
      },
      error: function (data) {
  ...
  function get_claim_info_ajax(p_car_number) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { car_number: p_car_number, ajax_action: 'get_claim_info' },
      success: function (data) {
        res = data
      },
      error: function (data) {
  ...
  function get_inv_cont_csl_ajax(p_front_end_id) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { front_end_id: p_front_end_id, ajax_action: 'get_inv_cont_csl' },
      success: function (data) {
  ...
  function create_return_invoice_ajax(
    p_car_number,
    p_otpr,
    p_graph,
    p_recip_address,
  ...
  function send_invoice_to_etran_ajax(p_car_number) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { car_number: p_car_number, ajax_action: 'send_invoice_to_etran' },
      success: function (data) {
        //alert(data);
        res = data
      },
  ...
  function close_return_invoice_ajax(p_car_number) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { car_number: p_car_number, ajax_action: 'close_return_invoice' },
      success: function (data) {
        res = data
      },
      error: function (data) {
  ...
  var md_div = $('<div/>').attr('title', 'Возврат порожняка').appendTo('body')
  ...
  md_div.inv_number = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
    css: { width: '100px' },
  }).prop('disabled', true)
  md_div.client_inv_number = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
    css: { width: '100px' },
  }).prop('disabled', true)
  ...
  if (l_claim_info.length == 1) {
    ...
    if (md_div.graph_pod.val() == '' || md_div.graph_pod.val() == null) {
      md_div.graph_pod
        .children('option[add_descr = "' + l_claim_info[0].GRAPH_ADD + '"]')
        .prop('selected', true)
    }
  }
  ...
  md_div.enter_claim_btn.click(function () {
    ...
    if (l_res.substr(0, 4) == 'done') {
      ...
      create_info_modal_dialog_new(
        'Предупреждение',
        'Процедура завершилась с ошибкой!!!' + '<br>' + l_res.substr(5),
      )
    }
    stop_loading_animation()
  })
  ...
  md_div.claim_otpr.change(function () {
    md_div.recip_telefon.val(
      md_div.claim_otpr.children('option:selected').attr('telefon'),
    )
  })
  ...
  md_div.disable_save_btn = function () {
    if (
      md_div.claim_otpr.val() == '' ||
      md_div.graph_pod.val() == '' ||
      md_div.recip_address.val() == '' ||
      md_div.recip_telefon.val() ==
        '' /*||(l_car_conts.length != 0&&md_div.depl_person.val()=='')*/
    ) {
  ...
  md_div.disable_send_to_etran_btn = function () {
    if (
      !r_send_invoice_to_etran ||
      md_div.client_inv_number.val() == '' ||
      md_div.depl_person.val() == ''
    ) {
      ...
      'Создать накладную': {
        ...
        click: function () {
          ...
          if (!l_exists_same_elements) {
            var l_cont_csl =
              l_car_conts.length != 0 ? cont_csl_table.get_rows() : ''

            start_loading_animation()

            var l_car_number = p_clicked_li.attr('data-id')

            var l_result = create_return_invoice_ajax(
              l_car_number,
              md_div.claim_otpr.val(),
              md_div.graph_pod.val(),
              md_div.recip_address.val(),
            ...
            if (l_result.substr(0, 4) == 'done') {
              ...
              create_info_modal_dialog_new(
                ...
                'Процедура завершилась успешно!' +
                  '<br>' +
                  'Создана накладная с клиентским идентификатором: ' +
                  l_result.substr(5),
              )
              ...
              create_info_modal_dialog_new(
                'Предупреждение',
                'Процедура завершилась с ошибкой!!! ' + l_result.substr(5),
              )
            }
          } else {
            create_info_modal_dialog_new(
              'Предупреждение',
              'Накладная не сохранилась, т.к. существуют ЗПУ с одинаковыми номерами!',
            )
          }
      ...
      'Отправить в ЭТРАН': {
        ...
        click: function () {
          ...
          if (l_result.substr(0, 4) == 'done') {
            ...
            create_info_modal_dialog_new(
              'Оповещение',
              'Процедура завершилась успешно!' + '<br>' + l_result.substr(5),
            )
          } else {
            create_info_modal_dialog_new(
              'Предупреждение',
...
function create_modal_dialog_list_notification_gu(p_type_gu) {
  ...
  function fill_select_notification(p_select) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { ajax_action: 'get_notifications_gu', type_gu: p_type_gu },
      success: function (data) {
        var records = JSON.parse(data)
        $.each(records, function (i, item) {
          var l_option = $('<option>').text(item.NAME).val(item.ID)
...
function create_modal_dialog_notification_gu(p_not_id, p_type_gu) {
  ...
  var gu_config = {
    '2b': {
      ...
      notification_time_label: 'Дата и время уведомления (Мск)',
      notification_time_fact_label:
        'Дата и время факта передачи уведомления (Мск)',
      notification_person_to_label: 'Кто принял',
      notification_time_fact_required: true,
      notification_person_to_required: true,
    ...
    '2d': {
      ...
      notification_time_label: 'Планируемая дата и время передачи (Мск)',
      notification_time_fact_label:
        'Дата и время составления уведомления (Местн)',
      notification_person_to_label: 'Кто принял',
      notification_time_fact_required: false,
      notification_person_to_required: false,
      show_etran_fields: true,
      show_contract_field: true,
  ...
  function update_notif_etran_ajax(p_not_id, p_pcalid) {
    var res
    $.ajax({
      url: 'data.php',
      type: 'POST',
      ...
      data: {
        ...
        type_gu: p_type_gu,
        pcalid: p_pcalid,
        ajax_action: 'update_notif_etran',
      },
      success: function (data) {
        res = data
      },
  ...
  function disable_save_btn() {
    if (
      md_div.notification_time.hasClass('red_bckg_color') ||
      md_div.notification_time.val() == '' ||
      md_div.notification_person_from.val() == '' ||
      md_div.notification_time_fact.hasClass('red_bckg_color') ||
      (gu_config[p_type_gu].notification_time_fact_required &&
        md_div.notification_time_fact.val() == '') ||
      (gu_config[p_type_gu].notification_person_to_required &&
  ...
  function railcar_table_const_for_not_gu() {
    ...
    return_table.add_carnumber_div = $('<div>', {
      css: { 'margin-left': '61px' },
    })
    return_table.append(
    ...
    return_table.cars_table = $('<table>', {
      class: 'received_cars_table gu_received_cars_table',
    }).append($('<tbody>'))
    return_table.append(
      $('<div>', {
        class: 'modalDialogContainer',
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            for (var i = 0; i < records.length; i++) {
              ...
              tr.up_image_div = $('<div>', { class: 'up_image' }).click(
                function () {
                  up_down_cars_table_tr('up', $(this).parent().parent())
                },
              )
              ...
              tr.down_image_div = $('<div>', { class: 'down_image' }).click(
                function () {
                  up_down_cars_table_tr('down', $(this).parent().parent())
                },
              )
              ...
              if (child.CONT_FROM_INV !== null) {
                ...
                l_conts.forEach(function (cont_num) {
                  var l_cont_descr = $('<div>', {
                    class: 'gu-item-cont-div',
                  }).appendTo(tr.cont_td)
                  $('<input>', {
                    class:
                      'gu-item-cont-input text ui-widget-content ui-corner-all',
                    readonly: 'true',
                    css: { background: '#DCDCDC' } /*,'disabled':true*/,
                  })
                    .val(cont_num)
                    .appendTo(l_cont_descr)
                })
              }
              ...
              if (child.CSL !== null) {
                ...
                l_csl_td.forEach(function (csl) {
                  ...
                  $('<input>', {
                    ...
                    .val(l_csl[0])
                    .appendTo(l_csl_descr)
                  $('<input>', {
                    class:
                      'gu-item-csl-type-input text ui-widget-content ui-corner-all',
                    readonly: 'true',
                    css: { background: '#DCDCDC' } /*,'disabled':true*/,
                  })
                    .val(l_csl[1])
                  ...
                  $('<input>', {
                    class:
                      'gu-item-csl-mark-input text ui-widget-content ui-corner-all',
                    readonly: 'true',
                    css: { background: '#DCDCDC' } /*,'disabled':true*/,
                  })
                    .val(l_csl[2])
              ...
              tr.del_image_div = $('<div>', {
                class: 'deleteImage deleteImage13px',
              }).click(function () {
  ...
  function run_report(p_not_id, p_user_name) {
    var win = window.open(
      'xx_etw_disl_003/xx_etw_disl_003.php?' +
        'not_id=' +
        p_not_id +
        '&' +
        'user_name=' +
  ...
  md_div.notification_num = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all required',
  }).attr('size', '20')
  md_div.not_number_etran = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
  })
    .attr('size', '20')
    .prop('disabled', true)
  md_div.not_state_etran = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
  })
    .attr('size', '20')
    .prop('disabled', true)
  md_div.notification_time = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all required',
  }).attr('size', '15')
  ...
  md_div.notification_time_fact = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
  }).attr('size', '15')
  md_div.notification_person_to = $('<input>', {
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
  }).attr('size', '20')

  if (gu_config[p_type_gu].notification_time_fact_required) {
    md_div.notification_time_fact.addClass('required')
  }
  if (gu_config[p_type_gu].notification_person_to_required) {
    md_div.notification_person_to.addClass('required')
  }

  var notification_attr_div = $('<div>', {
    class: 'attr',
    css: { border: 'none', width: '515px' },
  })
  ...
  if (gu_config[p_type_gu].show_etran_fields) {
    notification_attr_div
      .append(
        $('<div>')
          .append(
            $('<label>').text(gu_config[p_type_gu].notification_etran_label),
          )
              ...
              $('<div>', {
                ...
                .append(
                  $('<div>')
                    .append(
                      $('<label>').text(
                        gu_config[p_type_gu].notification_time_label,
                      ),
                    )
                ...
                .append(
                  $('<div>')
                    .append(
                      $('<label>').text('Фамилия передавшего уведомление'),
                    )
              ...
              $('<div>', {
                ...
                .append(
                  $('<div>')
                    .append(
                      $('<label>', {
                        css: { width: '320px', 'text-align': 'left' },
                      }).text(
                        gu_config[p_type_gu].notification_time_fact_label,
                      ),
                    )
                ...
                .append(
                  $('<div>')
                    .append(
                      $('<label>').text(
                        gu_config[p_type_gu].notification_person_to_label,
                      ),
                    )
  ...
  railcar_table.add_error_msg =
    'Регистрировать уведомление можно только для вагонов на станции Водороздельная!'
  // rem 31.01.2024 временно
  railcar_table.spec_check_car_number = function (p_car_number) {
    var find_result = $('#currentCarstree').find(
      'li[data-id=' + p_car_number + '][data-type="railcar"]',
    )
    if (find_result.length == 0) {
      return false
    }
  ...
  md_div.notification_num.blur(function () {
    disable_save_btn()
  })
  md_div.notification_person_from.change(function () {
    disable_save_btn()
  })
  md_div.notification_time.blur(function () {
    disable_save_btn()
  })
  md_div.notification_time_fact.blur(function () {
    disable_save_btn()
  })
  md_div.notification_person_to.blur(function () {
    disable_save_btn()
  })
  ...
  md_div.dialog({
    ...
    buttons: {
      ...
      'Обновить статус ЭТРАН': {
        text: 'Обновить статус ЭТРАН',
        id: 'md_update_etran_btn',
        click: function () {
          start_loading_animation()
          var l_pcalid = md_div.crg_pcalid_num.val()
          var l_result = update_notif_etran_ajax(p_not_id, l_pcalid)
          //console.log(l_result);
          if (l_result.substr(0, 4) == 'done') {
            create_info_modal_dialog_new(
              'Оповещение',
      ...
      Сохранить: {
        ...
        click: function () {
          ...
          if (check_open_period('1', md_div.notification_time.val()) == '0') {
            ...
            //console.log(cars_in_table);
            //return;
            var l_result = notification_gu_ajax(
              p_not_id,
              cars_in_table,
              md_div.notification_num.val(),
              md_div.notification_time.val(),
            ...
            if (l_result.substr(0, 4) == 'done') {
              ...
              var l_span_text =
                '(п/у ' + l_time.substr(0, 5) + ' ' + l_time.substr(11) + ')'
              cars_in_table_mas.forEach(function (car_num) {
                //alert(car_num);
                var l_car_in_tree = $('#currentCarstree').find(
                  'li[data-id=' + car_num + '][data-type="railcar"]',
                )
  ...
  if (p_type_gu == '2d' && md_div.not_number_etran.val() != '') {
    $('#md_update_etran_btn').show()
  } else if (p_type_gu == '2b' && md_div.not_number_etran.val() != '') {
    $('#md_update_etran_btn').show()
  } else {
    $('#md_update_etran_btn').hide()
  }
...
function create_md_output_cars() {
  ...
  function disable_save_btn() {
    if (
      md_div.custom_time.hasClass('red_bckg_color') ||
      md_div.custom_time.val() == '' ||
      md_div.comment.val() == ''
    ) {
      $('#md_ok_btn').prop('disabled', true)
    } else {
  ...
  md_div.append(
    $('<div>', { class: 'attr', css: { border: 'none', width: '440px' } })
      .append(
        $('<div>')
  ...
  md_div.custom_time.blur(function () {
    disable_save_btn()
  })
  md_div.comment.blur(function () {
    disable_save_btn()
  })
...
function create_md_cars_passport(p_feel_table) {
  function get_cars_passport_ajax(p_cars) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { cars: p_cars, ajax_action: 'get_cars_passport' },
      success: function (data) {
        res = data
      },
      error: function (data) {
  ...
  function run_report(p_car_number, p_user_name) {
    var win = window.open(
      'xx_car_nsi_report/xx_car_nsi_report.php?' +
        'car_number=' +
        p_car_number +
        '&' +
        'user_name=' +
  ...
  var l_add_btn = $('<button>', {
    class: 'button',
    css: { 'margin-left': '1em' },
  }).append(
    $('<span>', { class: 'button-text button-text-size-2' }).text('Добавить'),
  )
  var l_refresh_railcar_div = $('<div>', {
    class: 'border',
    css: { display: 'table' },
  ...
  $('<tr>')
    .appendTo(l_table.tbody)
    .append($('<td>').text('Собственник Наименование'))
  $('<tr>')
    .appendTo(l_table.tbody)
    .append($('<td>').text('Грузоподъемность(т)'))
  ...
  $('<tr>')
    .appendTo(l_table.tbody)
    .append($('<td>').text('Арендатор Наименование'))
  $('<tr>')
    .appendTo(l_table.tbody)
    .append($('<td>').text('Дата окончание аренды'))
  $('<tr>').appendTo(l_table.tbody).append($('<td>').text('Длина вагона(м)'))
  $('<tr>')
    .appendTo(l_table.tbody)
    .append($('<td>').text('Дата исключения из ГВЦ'))
  $('<tr>').appendTo(l_table.tbody).append($('<td>').text('Доп. информация'))
  $('<tr>')
    .appendTo(l_table.tbody)
    .append($('<td>').text('Дата обновления НСИ'))
  $('<tr>')
    .appendTo(l_table.tbody)
    .append($('<td>').text('Дата запроса(ЭТРАН)'))
  $('<tr>').appendTo(l_table.tbody).append($('<td>').text('Пользователь'))
  $('<tr>')
    .appendTo(l_table.tbody)
    .append($('<td>').text('Последний подход на Углеуральскую'))
  ...
  r_table.tbody = $('<tbody>').appendTo(r_table)
  r_table.add_car = function (p_car_info) {
    function get_value(p_num, p_car_info) {
      var l_text
      switch (p_num) {
        case 0:
          l_text = p_car_info.CAR_NUMBER
          break
        case 1:
          l_text = p_car_info.STATUS
          break
        case 2:
          l_text = p_car_info.CAR_LAST_REPAIR
          break
        case 3:
          l_text = p_car_info.CAR_NORMA
          break
        case 4:
          l_text = p_car_info.CAR_TYPE_NAME
          break
        case 5:
          l_text = p_car_info.CAR_MODEL
          break
        case 6:
          l_text = p_car_info.CAR_YEAR
          break
        case 7:
          l_text = p_car_info.CAR_AXLES
          break
        case 8:
          l_text = p_car_info.CAR_OWNER_COUNTRY_NAME
          break
        case 9:
          l_text = p_car_info.CAR_OWNER_TYPE_NAME
          break
        case 10:
          l_text = p_car_info.CAR_OWNER_OKPO
          break
        case 11:
          l_text = p_car_info.CAR_OWNER_NAME
          break
        case 12:
          l_text = p_car_info.CAR_TONNAGE
          break
        case 13:
          l_text = p_car_info.CAR_WEIGHT_DEP
          break
        case 14:
          l_text = p_car_info.CAR_VOLUME
          break
        case 15:
          l_text = p_car_info.CAR_ARENDATOR_OKPO
          break
        case 16:
          l_text = p_car_info.CAR_ARENDATOR_NAME
          break
        case 17:
          l_text = p_car_info.CAR_END_ARENDA_DATE
          break
        case 18:
          l_text = p_car_info.CAR_LENGTH
          break
        case 19:
          l_text = p_car_info.GVC_DATE
          break
        case 20:
          l_text = p_car_info.TEXT_TELEG
          break
        case 21:
          l_text = p_car_info.CAR_NSI_DATE
          break
        case 22:
          l_text = p_car_info.XX_LAST_UPDATE_DATE
          break
        case 23:
          l_text = p_car_info.XX_LAST_UPDATED_BY
          break
        case 24:
          l_text = p_car_info.UGL_DATE
          break
        default:
          l_text = null
          break
    ...
    function add_td(p_tr, p_num, p_car_info) {
      ...
      if (p_num == 0) {
        ...
        var l_div = $('<div>')
          .css({ position: 'relative', width: '140px', height: '20px' })
          .appendTo(l_td)
          .append(get_value(p_num, p_car_info))
        l_car_descr.remove_btn = $('<button>')
          .addClass('request-button cars-item-car-span-delete-button')
          .attr('title', 'Удалить вагон')
          .append(
            '<span class="request-button-icon delete-button-icon"></span>',
          )
          .appendTo(l_div)
          .click(function () {
      ...
      success: function (data) {
        ...
        $('<div/>', { class: 'context-menu context-menu-2lvl' }) // Присваиваем блоку наш css класс контекстного меню:
          ...
          .on('click', function (event) {
            ...
            create_window_undock_fix_device(
              p_railway_id,
              $(event.target).attr('data-id'),
            )
          })
      ...
      success: function (data) {
        ...
        $('<div/>', { class: 'context-menu context-menu-2lvl' }) // Присваиваем блоку наш css класс контекстного меню:
          ...
          .on('click', function (event) {
            ...
            create_window_update_fix_device(
              p_railway_id,
              $(event.target).attr('data-id'),
            )
          })
...
function create_window_add_fix_device(p_railway_id, p_part_id) {
  ...
  function railcar_table_for_fix_device() {
    ...
    return_table.add_carnumber_btn = $('<input>', {
      ...
      css: { 'font-size': '11px', height: '17px' },
      class: 'btnAdd',
    }).val('Добавить') // Назначение ЗУ (добавить)
    return_table.add_carnumber_btn.click(function () {
      //alert(1);
      l_car_number = return_table.add_carnumber_input.val()
      if (
        l_car_number.length == 0 ||
        (l_car_number == null && l_car_number !== '')
      ) {
        return_table.add_carnumber_input.addClass('red_bckg_color')
    ...
    return_table.cars_table = $('<table>', {
      class: 'received_cars_table fix_device_cars_table',
    }).append($('<tbody>'))
    return_table.append(
      $('<div>', {
        class: 'modalDialogContainer',
    ...
    function change_cars_table_total_tr() {
      ...
      if (self.cars_count !== 0) {
        ...
        return_table.cars_table_total_row.append('<td></td>')
        return_table.cars_table_total_row.append(
          '<td>' + Math.round(l_count_of_axles * 100) / 100 + '</td>',
        )
      } else {
        self.cars_lenght = 0
    ...
    this.add_cars_in_table = function (
      ...
      if (
        check_car_number_result &&
        ((add_car_number !== null && add_car_number !== '') ||
          p_railwai_id !== null ||
          p_railwai_id !== '')
      ) {
        //if (1 === 1) {
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            for (var i = 0; i < records.length; i++) {
              ...
              tr.append(
                '<td>' +
                  (child.COUNT_OF_AXLES !== null
                    ? child.COUNT_OF_AXLES.replace(',', '.')
                    : '') +
                  '</td>',
              )
  ...
  function table_fixing_device_list(p_railway_id, p_side_mas, p_side_type) {
    ...
    var l_refresh_btn = $('<div>', {
      class: 'refresh',
      title: 'Обновить',
      css: { position: 'relative', float: 'right', top: '1px', right: '2px' },
    })
    if (p_side_type == 'EVEN') {
          ...
          $.each(l_fd, function (i, item) {
            ...
            if (l_ul.find('li > input[id="' + item.ID + '"]').length === 0) {
              l_ul.append(
                $('<li>', {
                  class: 'ui-menu-item',
                  css: { color: l_text_color },
                  title: l_railway_name,
                })
                  .append(
                    $('<input>', {
                      id: item.ID,
            ...
            $('<div>')
              ...
              .append(
                $('<label>', {
                  text: '4_с',
                  class: '',
                  css: { 'margin-left': '3px', 'margin-right': '3px' },
                }),
              )
              .append(l_seg_4_f)
              .append(
  ...
  function table_norma_list(p_params) {
    ...
    l_device_rules.side_mas.forEach(function (item, index) {
      ...
      l_select = $('<select>', {
        id: id_select,
        class: 'route-window-attr-item-elem',
        css: { width: '15em' },
      })
      ...
      item.skid_mas.forEach(function (item) {
        var l_opt = $('<option>', {
          val: item.rule_id,
          norma: item.cnt_skid,
          cnt_axis: item.cnt_axis,
          text: '  норма:' + item.cnt_skid + ' осей: ' + item.cnt_axis,
        })
        if (item.selected === 'Y') {
      ...
      return_table
        ...
        .append(
          $('<label>', {
            text: item.side_descr,
            class:
              'route-window-attr-item-text route-window-attr-item-text-left',
            css: { 'padding-right': '10px', 'padding-left': l_css_even },
          }),
        )
        .append(l_select)
  ...
  function get_suitable_device_rules(p_params) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { params: p_params, ajax_action: 'get_suitable_device_rules' },
      success: function (data) {
        res = data
      },
      error: function (m1, m2) {
  ...
  function validation_fix(p_params) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { params: JSON.stringify(l_params), ajax_action: 'validation_fix' },
      success: function (data) {
        res = data
      },
      error: function (m1, m2) {
  ...
  function validation_save_fix() {
    ...
    if (l_over_val.length > 5) {
      create_info_modal_dialog_new(
        'Оповещение',
        'На сторонах назначены одинаковые ЗУ:' + l_over_val,
      )
      return false
    }
    ...
    if (dev_even_ar.length > 0) {
      create_info_modal_dialog_new(
        ...
        'На стороне "' +
          l_params.name_even +
          '" выбраны ЗУ, которые используются на других путях',
      )
      return false
    }
    ...
    if (calcul == 'AUTO') {
      ...
      if (
        $(l_time_fixing_even).attr('requir') == 'Y' &&
        $(l_time_fixing_even).val() == ''
      ) {
        create_info_modal_dialog_new(
          'Оповещение',
          'Не заполнено поле "Дата и время закрепления(Мск)" для стороны "' +
            l_params.name_even +
            '"',
        )
        return false
      ...
      if (
        $(l_who_fixing_even).attr('requir') == 'Y' &&
        $(l_who_fixing_even).val() == ''
      ) {
        create_info_modal_dialog_new(
          'Оповещение',
          'Не заполнено поле "Кто проводит" для стороны "' +
            l_params.name_even +
            '"',
        )
        return false
      ...
      if (
        $(l_who_fixing_report_even).attr('requir') == 'Y' &&
        $(l_who_fixing_report_even).val() == ''
      ) {
        create_info_modal_dialog_new(
          'Оповещение',
          'Не заполнено поле "Кому докладывает" для стороны "' +
            l_params.name_even +
            '"',
        )
        return false
      ...
      if (
        $(l_time_fixing_odd).attr('requir') == 'Y' &&
        $(l_time_fixing_odd).val() == ''
      ) {
        create_info_modal_dialog_new(
          'Оповещение',
          'Не заполнено поле "Дата и время закрепления(Мск)" для стороны "' +
      ...
      if (
        $(l_who_fixing_odd).attr('requir') == 'Y' &&
        $(l_who_fixing_odd).val() == ''
      ) {
        create_info_modal_dialog_new(
          'Оповещение',
          'Не заполнено поле "Кто проводит" для стороны "' +
      ...
      if (
        $(l_who_fixing_report_odd).attr('requir') == 'Y' &&
        $(l_who_fixing_report_odd).val() == ''
      ) {
        create_info_modal_dialog_new(
          'Оповещение',
          'Не заполнено поле "Кому докладывает" для стороны "' +
    ...
    if (calcul == 'MANUAL') {
      if (side_length == 1) {
        if (dev_even_count < 1) {
          create_info_modal_dialog_new(
            'Оповещение',
            'На стороне "' + l_params.name_even + '" не выбраны ЗУ!',
          )
          return false
        }
        if (
        ...
        if (dev_odd_count < 1) {
          create_info_modal_dialog_new(
            'Оповещение',
            'На стороне "' + l_params.name_odd + '" не выбраны ЗУ!',
          )
          return false
        }
  ...
  function get_overall_array(p_array_first, p_array_second) {
    ...
    $.each(p_array_first, function (key_1, value_1) {
      $.each(p_array_second, function (key_2, value_2) {
        if (
          p_array_first[key_1]['device_id'] ==
          p_array_second[key_2]['device_id']
        ) {
  ...
  var md_content = $('<div/>').addClass('md-lvl-1').appendTo('body')

  var l_title = 'Назначение ЗУ на путь'

  md_content.attr('title', l_title)
  md_content.railway_input = $('<input>', {
    disabled: '',
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
    css: { width: 'auto' },
  }).val(railway_name)
  md_content.part_input = $('<input>', {
    disabled: '',
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
    css: { width: 'auto' },
  }).val(railway_parts_name)
  md_content.users = $('<input>', {
    disabled: '',
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
    css: { width: 'auto' },
  }).val(user_name)
  md_content.server_time = $('<input>', {
    disabled: '',
    type: 'text',
    class: 'text ui-widget-content ui-corner-all',
    css: { width: 'auto' },
  }).val(get_server_current_time())

  md_content.choose_norm = $('<input>', {
    type: 'button',
    id: 'md_choose_norm_btn',
    value: 'Подобрать норму',
    class: 'md_save_load',
  })

  md_content.time_fixing_even = $('<input>', {
    type: 'text',
    requir: 'Y',
    class: 'text ui-widget-content ui-corner-all required',
  }).attr('size', '15')
  ...
  md_content.time_fixing_odd = $('<input>', {
    type: 'text',
    requir: 'Y',
    class: 'text ui-widget-content ui-corner-all required',
  }).attr('size', '15')
  ...
  var l_div_railway = $('<div>')
    ...
    .append(
      $('<label>', {
        text: 'Наименование',
        class: 'route-window-attr-item-text route-window-attr-item-text-left',
      }),
    )
  ...
  var l_div_choose_norm = $('<div>').addClass(
    'route-window-attr-item helper-clearfix',
  )
  //.append(md_content.choose_norm);

  var l_div_info = $('<div>', {
    css: { display: 'table', float: 'left', 'padding-right': '10px' },
  }).append(
    $('<div>', { class: 'border', css: { clear: 'both' } }).append(
      $('<div>', { css: { display: 'inline-table' } }).append(
  ...
  var l_div_norm = $('<div>', { css: { display: 'table' } }).append(
    $('<div>', {
      class: 'border',
      css: { clear: 'both', border: 'none' },
    }).append(
      $('<div>', { css: { display: 'inline-table' } }).append(
        $('<div>', {
          class: 'attr',
  ...
  var l_div_time_fixing = $('<div>')
    ...
    .append(
      $('<label>', {
        text: 'Дата и время закрепления(Мск)',
        class: 'route-window-attr-item-text route-window-attr-item-text-left',
      }),
    )
  ...
  var fixing_device_even_list = new table_fixing_device_list(
    p_railway_id,
    l_side_mas,
    'EVEN',
  ) /* Башмаки с четной стороны */
  var fixing_device_odd_list = new table_fixing_device_list(
    p_railway_id,
    l_side_mas,
    'ODD',
  ) /* Башмаки с нечетной стороны */

  /* Четная сторона */
  var l_div_even_side = $('<div>', {
    css: { display: 'table', float: 'left', 'padding-right': '10px' },
  }).append(
    $('<div>', { class: 'border', css: { clear: 'both' } })
      .append(
        $('<div>', { class: 'header', css: { width: '380px' } }).text(
          l_side_name_even,
        ),
      )
      .append(
  ...
  /* Нечетная сторона */
  var l_div_odd_side = $('<div>', { css: { display: 'table' } }).append(
    $('<div>', { class: 'border', css: { clear: 'both' } })
      .append(
        $('<div>', { class: 'header', css: { width: '380px' } }).text(
          l_side_name_odd,
    ...
    buttons: {
      ...
      'Сохранить норму': {
        ...
        click: function () {
          ...
          v_params.device_mas_even =
            fixing_device_even_list.get_fixing_device_in_list() // ЧетС.Список башмаков
          v_params.device_mas_odd =
            fixing_device_odd_list.get_fixing_device_in_list() // НеЧетС.Список башмаков
...
function create_window_undock_fix_device(p_railway_id, p_part_id) {
  ...
  /* Список башмаков(chexbox) */
  function table_fixing_device_list(
    p_railway_id,
    part_id,
    p_side_mas,
    p_side_type,
  ...
  function save_transaction_ondock(p_add_data) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { add_data: p_add_data, ajax_action: 'save_transaction_ondock' },
      success: function (data) {
        //console.log(data);
        res = data
      },
  ...
  function validation_save_ondock() {
    ...
    if (
      ...
      if (!date_comparison(l_date_fix_even, l_time_fixing_even.val(), '<')) {
        create_info_modal_dialog_new(
          'Оповещение',
          '"Дата и время закрепления(Мск)" больше "Дата и время снятия(Мск)"! "Дата и время закрепления(Мск) = ' +
            l_date_fix_even,
        )
        return false
      }
  ...
  var l_div_who_report_odd = $('<div>')
    ...
    .append(md_content.who_report_odd)
  var l_div_note = $('<div>', { css: { display: 'table' } }).append(
    $('<div>', { class: 'attr', css: { border: 'none' } }).append(
      $('<div>')
        .append(
          $('<label>', {
            text: 'Примечание',
    ...
    buttons: {
      Сохранить: {
        ...
        click: function () {
          ...
          v_params.device_mas_even =
            fixing_device_even_list.get_fixing_device_out_list() // ЧетС.Список башмаков
          v_params.device_mas_odd =
            fixing_device_odd_list.get_fixing_device_out_list() // НеЧетС.Список башмаков
          v_params.time_fixing_even = md_content.time_fixing_even.val() // ЧетС.Дата
          v_params.who_fixing_even = md_content.who_fixing_even.val() // ЧетС.Кто проводит
          v_params.who_fixing_report_even = md_content.who_report_even.val() // ЧетС.Кому докладывает
          v_params.who_fixing_odd = md_content.who_fixing_odd.val() // НечС.Кто проводит
...
function create_window_update_fix_device(p_railway_id, p_part_id) {
  ...
  function set_users_for_cond_train(p_select, p_params, p_type, p_user_type) {
    ...
    $.ajax({
      ...
      async: false,
      data: {
        params: JSON.stringify(v_params),
  ...
  function validation_save_fix() {
    ...
    if (
      $(l_time_fixing_even).val() !== null &&
      $(l_time_fixing_even).val() !== '' &&
      $(l_time_fixing_odd).val() !== null &&
      $(l_time_fixing_odd).val() !== ''
    ) {
    ...
    if (calcul == 'MANUAL') {
      if (side_length == 1) {
        if (dev_even_count < 1) {
          create_info_modal_dialog_new(
            'Оповещение',
            'На стороне "' + v_params.name_even + '" не выбраны ЗУ!',
          )
          return false
        }
        if (
        ...
        if (dev_odd_count < 1) {
          create_info_modal_dialog_new(
            'Оповещение',
            'На стороне "' + v_params.name_odd + '" не выбраны ЗУ!',
          )
          return false
        }
  ...
  function delete_fix(p_params) {
    ...
    $.ajax({
      ...
      dataType: 'text',
      async: false,
      data: { params: p_params, ajax_action: 'delete_fix' },
      success: function (data) {
        res = data
      },
      error: function (m1, m2) {
  ...
  init_date_time_input(md_content.time_fixing_even)
  set_header_value_fix(
    l_params,
    md_content.time_fixing_even,
    'time_fixing_even',
  )
  init_date_time_input(md_content.time_fixing_odd)
  set_header_value_fix(l_params, md_content.time_fixing_odd, 'time_fixing_odd')

  set_users_for_cond_train(
    md_content.who_fixing_even,
    l_params,
    'prc_fc',
    'USER_FIX_EVEN',
  )
  set_users_for_cond_train(
    md_content.who_fixing_odd,
    l_params,
    'prc_fc',
    'USER_FIX_ODD',
  )
  ...
  set_users_for_cond_train(
    md_content.who_report_even,
    l_params,
    'prr_fc',
    'USER_FIX_REP_EVEN',
  ) // Кому докладывает
  set_users_for_cond_train(
    md_content.who_report_odd,
    l_params,
    'prr_fc',
    'USER_FIX_REP_ODD',
  ) // Кому докладывает
...
function create_md_processing_of_wagons() {
  ...
  m_win_div.begin_processing_date = $('<input>', {
    type: 'text',
    requir: 'Y',
    class: 'text ui-widget-content ui-corner-all required',
  }).attr('size', '15')
  var l_div_begin_processing_date = $('<div>')
    ...
    .append(
      $('<label>', {
        text: 'Дата/время начала обработки',
        class:
          'processing-wagons-window-attr-item-text processing-wagons-window-attr-item-text-left',
      }),
    )
    .append(m_win_div.begin_processing_date)

  // Дата/время окончания обработки
  m_win_div.end_processing_date = $('<input>', {
    type: 'text',
    requir: 'Y',
    class: 'text ui-widget-content ui-corner-all required',
  }).attr('size', '15')
  var l_div_end_processing_date = $('<div>')
    ...
    .append(
      $('<label>', {
        text: 'Дата/время окончания обработки',
        class:
          'processing-wagons-window-attr-item-text processing-wagons-window-attr-item-text-left',
      }),
    )
  ...
  //Добавить вагон
  m_win_div.processing_add_wagon = $('<input>', {
    type: 'text',
    css: { 'font-size': '11px' },
    class: 'text ui-widget-content ui-corner-all',
  })
    .attr('size', '11')
    .attr('maxlength', '11')
  m_win_div.processing_add_wagon_btn = $('<input>', {
    type: 'button',
    css: { 'font-size': '11px', height: '17px' },
    class: 'btnAdd',
  }).val('Добавить')
  ...
  m_win_div
    .append(l_div_begin_processing_date)
  ...
  function get_tr_for_processing_table(
    /*p_number,p_obj_number,p_freight,p_cont*/ p_obj_car,
  ) {
    ...
    $(tr).attr(
      'disl_lines_id',
      p_obj_car.disl_lines_id === null ? '0' : p_obj_car.disl_lines_id,
    )

    // Если по строке интерфейс отработал без ошибок, то чекбокс неактивный
    if (p_obj_car.interface_status !== 'Y') {
      tr.append(
        $('<td>').append($(ProcessingRailway).attr('checked', 'checked')),
      )
    } else {
    ...
    tr.append(
      $('<td>').text(p_obj_car.car_number === null ? '' : p_obj_car.car_number),
    ) //,{'text':p_obj_car.car_number}));
    tr.append(
      $('<td>').text(p_obj_car.cont_num === null ? '' : p_obj_car.cont_num),
    ) //,{'text':p_obj_car.cont_num}));
    tr.append(
      $('<td>').text(
        p_obj_car.freight_name === null ? '' : p_obj_car.freight_name,
      ),
    ) //,{'text':p_obj_car.freight_name}));

    tr.append($('<td>').append(select_freight))
    tr.append(
      $('<td>').text(p_obj_car.batch_num === null ? '' : p_obj_car.batch_num),
    )
    tr.append(
      $('<td>').text(
        p_obj_car.trx_date_begin_text === null
          ? ''
          : p_obj_car.trx_date_begin_text,
      ),
    )
    tr.append(
      $('<td>').text(
        p_obj_car.trx_date_end_text === null ? '' : p_obj_car.trx_date_end_text,
      ),
    )
  ...
  function CheckCountCheckbox() {
    return $(m_win_div.TableProcessingWagons).find(
      'input[name="ProcessingRailway"]:checked',
    ).length
  }
  ...
  function md_enter_processing_ajax(data_obj) {
    ...
    $.ajax({
      ...
      async: false,
      data: {
        params: JSON.stringify(data_obj),
  ...
  function ValidatingRequiredValues(ObjArray) {
    ...
    for (var i = 0; i < ObjArray.opm_trx_lines.length; i++) {
      if (
        ObjArray.opm_trx_lines[i].item_id === '0' ||
        ObjArray.opm_trx_lines[i].item_id === 0
      ) {
    ...
    if (ItemCount !== 0) {
      create_info_modal_dialog_new(
        'Оповещение',
        'Не выбрана позиция для строки:' + ItemMeassge.slice(0, -1),
      )
      return false
    }
    if (CheckCountCheckbox() === 0) {
      create_info_modal_dialog_new(
        'Оповещение',
        'Не выбраны строки для обработки!',
      )
      return false
    }
    if (
      m_win_div.begin_processing_date.attr('requir') == 'Y' &&
      m_win_div.begin_processing_date.val() == ''
    ) {
      create_info_modal_dialog_new(
        'Оповещение',
        'Не заполнено поле "Дата/время начала обработки"',
      )
      return false
    }
    if (
      m_win_div.begin_processing_date.attr('requir') == 'Y' &&
      m_win_div.end_processing_date.val() == ''
    ) {
      create_info_modal_dialog_new(
        'Оповещение',
        'Не заполнено поле "Дата/время окончание обработки"',
      )
      return false
    }
    return true
    ...
    buttons: {
      ...
      'Запустить интерфейс': {
        ...
        click: function () {
          ...
          stop_loading_animation()
          if (f_res_mas[0] === 'done') {
            // upd 20.06.2022 add f_res_mas[0]
            create_info_modal_dialog_new(
              'Оповещение',
              'Процедура завершилась успешно!' + f_res_mas[1],
            )
            $(this).dialog('close')
          } else {
            create_info_modal_dialog_new(
              'Ошибка',
              'Процедура завершилась с ошибкой!!!' + f_res_mas[1],
            ) // upd 20.06.2022 add res[1]
          }
        },
      },
      Закрыть: function () {
        if (CheckCountCheckbox() !== 0) {
          create_info_modal_dialog_btn(
            'Оповещение',
...
function create_md_manual_upd_wagon() {
  ...
  //Добавить вагон
  m_win_div.manual_upd_add_wagon = $('<input>', {
    type: 'text',
    css: { 'font-size': '11px' },
    class: 'text ui-widget-content ui-corner-all',
  })
    .attr('size', '8')
    .attr('maxlength', '8')
  m_win_div.manual_upd_add_wagon_btn = $('<input>', {
    type: 'button',
    css: { 'font-size': '11px', height: '17px' },
    class: 'btnAdd',
  }).val('Добавить')
  ...
  function add_wagon_in_table(p_type_obj) {
    ...
    if (obj_number.length < 8) {
      ...
      proc_tbl.find('tr td:contains("' + obj_number + '")').length !== 0
    ) {
      wagon_number_input.addClass('red_bckg_color')
      alert('Вагон ' + obj_number + ' уже добавлен!')
    } else {
...
function get_processings_of_wagons() {
  ...
  var l_div_begin_processing_date = $('<div>')
    ...
    .append(
      $('<label>', {
        text: 'Дата/время начала интервала',
        class:
          'processing-wagons-window-attr-item-text processing-wagons-window-attr-item-text-left',
      }),
    )
  ...
  var l_div_end_processing_date = $('<div>')
    ...
    .append(
      $('<label>', {
        text: 'Дата/время окончания интервала',
        class:
          'processing-wagons-window-attr-item-text processing-wagons-window-attr-item-text-left',
      }),
    )
  ...
  function get_tr_processing(obj) {
    ...
    tr.append(
      $('<td>').text(
        obj.begin_transaction_date === null ? '' : obj.begin_transaction_date,
      ),
    )
    tr.append(
      $('<td>').text(
        obj.end_transaction_date === null ? '' : obj.end_transaction_date,
      ),
    )
    var qty_production = obj.qty_production === null ? '' : obj.qty_production
    tr.append($('<td>').text(qty_production))
    var qty_write_off =
      obj.qty_write_off === null ? '' : Math.abs(obj.qty_write_off)
...
function entry_foreign_container() {
  ...
  function entryContainer() {
    ...
    this.check_container = function () {
      ...
      if (
        return_table.find('tr td:contains("' + p_cont_number + '")').length !==
        0
      ) {
  ...
  m_win_div.div_date_oper = $('<div/>').append(
    '<div>' +
      '<label for="modalDialogDateFact">Дата и время операции</label>' +
      '<input id="modalDialogDateFact" type="text" size="14" class="text ui-widget-content ui-corner-all">' +
      '</div>',
  ...
  m_win_div.new_place_station.append(
    $('<option>').val('2').text('Водораздельная'),
  )
  m_win_div.new_place_station.append($('<option>').val('3').text('Новая'))

  m_win_div.new_place_station.val(
    $('#currentCarstree li[data-type="station"]').attr('data-id'),
  )
  ...
  m_win_div.div_new_place = $('<div/>').append(
    $('<div>', { css: { display: 'table' } }).append(
      $('<div>', { class: 'border', css: { clear: 'both' } })
        .append(
          $('<div>', { class: 'header', css: { width: '180px' } }).text(
          ...
          $('<div>', { css: { display: 'inline-table' } }).append(
            $('<div>', {
              ...
              $('<div>')
                .append(
                  $('<div>', { css: { 'text-align': 'left' } }).append(
                    m_win_div.new_place_type,
                  ),
                )
                .append(
                  $('<div>', {
                    css: { 'text-align': 'left', 'margin-top': '5px' },
                  }).append(m_win_div.new_place_station),
                )
                .append(
                  $('<div>', {
                    css: { 'text-align': 'left', 'margin-top': '5px' },
                  }).append(m_win_div.new_place),
                ),
            ),
          ),
        ),
  ...
  m_win_div.btn_add.click(function () {
    ...
    if (m_win_div.new_place.val() === '') {
      create_info_modal_dialog_new(
        'Ошибка',
        'Не заполнено поле "Место назначение"!',
      )
      return
    }
  ...
  m_win_div
    ...
    .append($('<div>').append(m_win_div.btn_add).append(m_win_div.btn_delete))
